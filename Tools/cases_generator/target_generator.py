...
def write_opcode_targets(analysis: Analysis, out: CWriter) -> None:
    ...
    out.emit("#if !Ty_TAIL_CALL_INTERP\n")
    out.emit("static void *opcode_targets[256] = {\n")
    for target in targets:
        out.emit(target)
    out.emit("};\n")
    out.emit("#else /* Ty_TAIL_CALL_INTERP */\n")

def function_proto(name: str) -> str:
    return f"Ty_PRESERVE_NONE_CC static PyObject *_TAIL_CALL_{name}(TAIL_CALL_PARAMS)"
...
def write_tailcall_dispatch_table(analysis: Analysis, out: CWriter) -> None:
    ...
    out.emit("};\n")
    outfile.write("#endif /* Ty_TAIL_CALL_INTERP */\n")
