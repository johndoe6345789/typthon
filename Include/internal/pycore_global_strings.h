#ifndef Ty_INTERNAL_GLOBAL_STRINGS_H
#define Ty_INTERNAL_GLOBAL_STRINGS_H
#ifdef __cplusplus
extern "C" {
#endif
...
#include "pycore_global_objects.h"  // struct _Ty_SINGLETON
...
#define _Ty_ID(NAME) \
     (_Ty_SINGLETON(strings.identifiers._py_ ## NAME._ascii.ob_base))
#define _Ty_STR(NAME) \
     (_Ty_SINGLETON(strings.literals._py_ ## NAME._ascii.ob_base))
#define _Ty_LATIN1_CHR(CH) \
    ((CH) < 128 \
     ? (TyObject*)&_Ty_SINGLETON(strings).ascii[(CH)] \
     : (TyObject*)&_Ty_SINGLETON(strings).latin1[(CH) - 128])

/* _Ty_DECLARE_STR() should precede all uses of _Ty_STR() in a function.
   ...
   Pairing _Ty_DECLARE_STR() with every use of _Ty_STR() makes sure the
...
#define _Ty_DECLARE_STR(name, str)

#ifdef __cplusplus
}
#endif
#endif /* !Ty_INTERNAL_GLOBAL_STRINGS_H */
