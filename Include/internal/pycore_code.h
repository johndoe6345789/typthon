#ifndef Ty_INTERNAL_CODE_H
#define Ty_INTERNAL_CODE_H
#ifdef __cplusplus
extern "C" {
#endif
...
#include "pycore_backoff.h"     // _Ty_BackoffCounter
...
#define _Ty_OPCODE(word) ((word).op.code)
#define _Ty_OPARG(word) ((word).op.arg)
...
static inline void
_py_set_opcode(_Ty_CODEUNIT *word, uint8_t opcode)
{
    word->op.code = opcode;
}

#define _Ty_MAKE_CODEUNIT(opcode, oparg) _py_make_codeunit((opcode), (oparg))
#define _Ty_SET_OPCODE(word, opcode) _py_set_opcode(&(word), (opcode))


// We hide some of the newer PyCodeObject fields behind macros.
// This helps with backporting certain changes to 3.12.
#define _TyCode_HAS_EXECUTORS(CODE) \
    (CODE->co_executors != NULL)
#define _TyCode_HAS_INSTRUMENTATION(CODE) \
...
#define CACHE_ENTRIES(cache) (sizeof(cache)/sizeof(_Ty_CODEUNIT))

typedef struct {
    _Ty_BackoffCounter counter;
...
_PyLocals_GetKind(TyObject *kinds, int i)
{
    assert(TyBytes_Check(kinds));
    assert(0 <= i && i < TyBytes_GET_SIZE(kinds));
    char *ptr = TyBytes_AS_STRING(kinds);
    return (_PyLocals_Kind)(ptr[i]);
}

static inline void
_PyLocals_SetKind(TyObject *kinds, int i, _PyLocals_Kind kind)
{
    assert(TyBytes_Check(kinds));
    assert(0 <= i && i < TyBytes_GET_SIZE(kinds));
    char *ptr = TyBytes_AS_STRING(kinds);
...
struct _PyCodeConstructor {
    ...
    TyObject *localsplusnames;  // Tuple of strings
    TyObject *localspluskinds;  // Bytes object, one byte per variable
...
extern int _TyCode_Validate(struct _PyCodeConstructor *);
extern PyCodeObject* _TyCode_New(struct _PyCodeConstructor *);


/* Private API */

/* Getters for internal PyCodeObject data. */
extern TyObject* _TyCode_GetVarnames(PyCodeObject *);
extern TyObject* _TyCode_GetCellvars(PyCodeObject *);
extern TyObject* _TyCode_GetFreevars(PyCodeObject *);
extern TyObject* _TyCode_GetCode(PyCodeObject *);

/** API for initializing the line number tables. */
extern int _TyCode_InitAddressRange(PyCodeObject* co, PyCodeAddressRange *bounds);
...
/** API for executors */
extern void _TyCode_Clear_Executors(PyCodeObject *code);
...
extern void _Ty_Specialize_LoadSuperAttr(_PyStackRef global_super, _PyStackRef cls,
                                         _Ty_CODEUNIT *instr, int load_method);
extern void _Ty_Specialize_LoadAttr(_PyStackRef owner, _Ty_CODEUNIT *instr,
                                    TyObject *name);
extern void _Ty_Specialize_StoreAttr(_PyStackRef owner, _Ty_CODEUNIT *instr,
                                     TyObject *name);
extern void _Ty_Specialize_LoadGlobal(TyObject *globals, TyObject *builtins,
                                      _Ty_CODEUNIT *instr, TyObject *name);
extern void _Ty_Specialize_StoreSubscr(_PyStackRef container, _PyStackRef sub,
                                       _Ty_CODEUNIT *instr);
extern void _Ty_Specialize_Call(_PyStackRef callable, _Ty_CODEUNIT *instr,
                                int nargs);
extern void _Ty_Specialize_CallKw(_PyStackRef callable, _Ty_CODEUNIT *instr,
                                  int nargs);
extern void _Ty_Specialize_BinaryOp(_PyStackRef lhs, _PyStackRef rhs, _Ty_CODEUNIT *instr,
                                    int oparg, _PyStackRef *locals);
extern void _Ty_Specialize_CompareOp(_PyStackRef lhs, _PyStackRef rhs,
                                     _Ty_CODEUNIT *instr, int oparg);
extern void _Ty_Specialize_UnpackSequence(_PyStackRef seq, _Ty_CODEUNIT *instr,
                                          int oparg);
extern void _Ty_Specialize_ForIter(_PyStackRef iter, _Ty_CODEUNIT *instr, int oparg);
extern void _Ty_Specialize_Send(_PyStackRef receiver, _Ty_CODEUNIT *instr);
extern void _Ty_Specialize_ToBool(_PyStackRef value, _Ty_CODEUNIT *instr);
extern void _Ty_Specialize_ContainsOp(_PyStackRef value, _Ty_CODEUNIT *instr);
extern void _Ty_GatherStats_GetIter(_PyStackRef iterable);
...
static inline _Ty_BackoffCounter
adaptive_counter_backoff(_Ty_BackoffCounter counter) {
...
typedef int (*binaryopguardfunc)(TyObject *lhs, TyObject *rhs);
typedef TyObject *(*binaryopactionfunc)(TyObject *lhs, TyObject *rhs);
...
extern TyObject *_PyInstrumentation_BranchesIterator(PyCodeObject *code);

struct _PyCode8 _TyCode_DEF(8);

PyAPI_DATA(const struct _PyCode8) _Ty_InitCleanup;

#ifdef Ty_GIL_DISABLED

static inline _PyCodeArray *
_TyCode_GetTLBCArray(PyCodeObject *co)
{
    return _Ty_STATIC_CAST(_PyCodeArray *,
                           _Ty_atomic_load_ptr_acquire(&co->co_tlbc));
}
...
_TyCode_GetTLBCFast(TyThreadState *tstate, PyCodeObject *co)
{
    ...
    if (idx < code->size && code->entries[idx] != NULL) {
        return (_Ty_CODEUNIT *) code->entries[idx];
    }
    return NULL;
}
...
extern _Ty_CODEUNIT *_TyCode_GetTLBC(PyCodeObject *co);
...
PyAPI_FUNC(void) _TyCode_GetVarCounts(
        PyCodeObject *,
        _TyCode_var_counts_t *);
...
PyAPI_FUNC(int) _TyCode_CheckNoInternalState(PyCodeObject *, const char **);
PyAPI_FUNC(int) _TyCode_CheckNoExternalState(
        PyCodeObject *,
        _TyCode_var_counts_t *,
        const char **);
...
PyAPI_FUNC(int) _TyCode_CheckPureFunction(PyCodeObject *, const char **);
PyAPI_FUNC(int) _TyCode_ReturnsOnlyNone(PyCodeObject *);


#ifdef __cplusplus
}
#endif
#endif /* !Ty_INTERNAL_CODE_H */
