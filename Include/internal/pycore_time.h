...
// handle exceptions. For example, _TyTime_Add(t1, t2) computes t1+t2 and
...
#ifndef Ty_INTERNAL_TIME_H
#define Ty_INTERNAL_TIME_H
#ifdef __cplusplus
extern "C" {
#endif
...
typedef enum {
    ...
    _TyTime_ROUND_FLOOR=0,

    // Round towards infinity (+inf).
    // For example, used for timeout to wait "at least" N seconds.
    _TyTime_ROUND_CEILING=1,

    // Round to nearest with ties going to nearest even integer.
    // For example, used to round from a Python float.
    _TyTime_ROUND_HALF_EVEN=2,

    // Round away from zero
    // For example, used for timeout. _TyTime_ROUND_CEILING rounds
    // -1e-9 to 0 milliseconds which causes bpo-31786 issue.
    // _TyTime_ROUND_UP rounds -1e-9 to -1 millisecond which keeps
    // the timeout sign as expected. select.poll(timeout) must block
    // for negative values.
    _TyTime_ROUND_UP=3,

    // _TyTime_ROUND_TIMEOUT (an alias for _TyTime_ROUND_UP) should be
    // used for timeouts.
    _TyTime_ROUND_TIMEOUT = _TyTime_ROUND_UP
} _TyTime_round_t;


// Convert a time_t to a PyLong.
// Export for '_testinternalcapi' shared extension
PyAPI_FUNC(TyObject*) _TyLong_FromTime_t(time_t sec);

// Convert a PyLong to a time_t.
// Export for '_datetime' shared extension
PyAPI_FUNC(time_t) _TyLong_AsTime_t(TyObject *obj);

// Convert a number of seconds, int or float, to time_t.
// Export for '_datetime' shared extension.
PyAPI_FUNC(int) _TyTime_ObjectToTime_t(
    TyObject *obj,
    time_t *sec,
    _TyTime_round_t);
...
// Export for '_datetime' shared extension.
PyAPI_FUNC(int) _TyTime_ObjectToTimeval(
    TyObject *obj,
    time_t *sec,
    long *usec,
    _TyTime_round_t);
...
// Export for '_testinternalcapi' shared extension.
PyAPI_FUNC(int) _TyTime_ObjectToTimespec(
    TyObject *obj,
    time_t *sec,
    long *nsec,
    _TyTime_round_t);
...
// us is always positive. This function is similar to _TyTime_AsTimeval()
...
// Return 0 on success, raise an exception and return -1 on error.
// Export for '_datetime' shared extension.
PyAPI_FUNC(int) _TyTime_localtime(time_t t, struct tm *tm);

// Converts a timestamp to the Gregorian time, assuming UTC.
// Return 0 on success, raise an exception and return -1 on error.
// Export for '_datetime' shared extension.
PyAPI_FUNC(int) _TyTime_gmtime(time_t t, struct tm *tm);
