...
#ifndef Ty_INTERNAL_RUNTIME_STRUCTS_H
#define Ty_INTERNAL_RUNTIME_STRUCTS_H
#ifdef __cplusplus
extern "C" {
#endif

#include "pycore_interp_structs.h" // _TyGC_Head_UNUSED
...
struct _Ty_float_runtime_state {
...
#include "pycore_debug_offsets.h" // _Ty_DebugOffsets
...
struct _Ty_cached_objects {
    // XXX We could statically allocate the hashtable.
    _Ty_hashtable_t *interned_strings;
};
...
#include "pycore_global_strings.h" // struct _Ty_global_strings

struct _Ty_static_objects {
    struct {
        ...
        struct _Ty_global_strings strings;

        _TyGC_Head_UNUSED _tuple_empty_gc_not_used;
        PyTupleObject tuple_empty;

        _TyGC_Head_UNUSED _hamt_bitmap_node_empty_gc_not_used;
...
struct pyruntimestate {
    ...
    /* Is running Ty_PreInitialize()? */
    int preinitializing;

    /* Is Python preinitialized? Set to 1 by Ty_PreInitialize() */
    int preinitialized;

    /* Is Python core initialized? Set to 1 by _Ty_InitializeCore() */
    int core_initialized;

    /* Is Python fully initialized? Set to 1 by Ty_Initialize() */
    int initialized;

    /* Set by Ty_FinalizeEx(). Only reset to NULL if Ty_Initialize()
       is called again.
    ...
    struct pyinterpreters {
        ...
        /* next_id is an auto-numbered sequence of small
           ...
           TyInterpreterState_New().  A negative interpreter ID
    ...
    Ty_OpenCodeHookFunction open_code_hook;
    void *open_code_userdata;
    struct {
        PyMutex mutex;
        struct _Ty_AuditHookEntry *head;
    } audit_hooks;

    struct _py_object_runtime_state object_state;
    struct _Ty_float_runtime_state float_state;
    struct _Ty_unicode_runtime_state unicode_state;
    struct _types_runtime_state types;
    struct _Ty_time_runtime_state time;
    ...
    struct _Ty_cached_objects cached_objects;
    struct _Ty_static_objects static_objects;
...
#endif
#endif /* Ty_INTERNAL_RUNTIME_STRUCTS_H */
