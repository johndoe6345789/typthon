...
typedef struct {
    TyTypeObject *dbm_type;
    TyObject *dbm_error;
} _dbm_state;

static inline _dbm_state*
get_dbm_state(TyObject *module)
{
...
#define check_dbmobject_open(v, err)                                \
    if ((v)->di_dbm == NULL) {                                      \
        TyErr_SetString(err, "DBM object has already been closed"); \
        return NULL;                                                \
    }
...
newdbmobject(_dbm_state *state, const char *file, int flags, int mode)
{
    ...
    if ( (dp->di_dbm = dbm_open((char *)file, flags, mode)) == 0 ) {
        TyErr_SetFromErrnoWithFilename(state->dbm_error, file);
        Ty_DECREF(dp);
        return NULL;
    }
    return (TyObject *)dp;
...
/* Methods */
static int
dbm_traverse(TyObject *dp, visitproc visit, void *arg)
{
    Ty_VISIT(Ty_TYPE(dp));
    return 0;
}
...
static Ty_ssize_t
dbm_length(TyObject *self)
{
    dbmobject *dp = dbmobject_CAST(self);
    _dbm_state *state = TyType_GetModuleState(Ty_TYPE(dp));
    assert(state != NULL);
    if (dp->di_dbm == NULL) {
             TyErr_SetString(state->dbm_error, "DBM object has already been closed");
             return -1;
    }
    if ( dp->di_size < 0 ) {
        datum key;
...
static TyObject *
dbm_subscript(TyObject *self, TyObject *key)
{
    datum drec, krec;
    Ty_ssize_t tmp_size;
    dbmobject *dp = dbmobject_CAST(self);
    _dbm_state *state = TyType_GetModuleState(Ty_TYPE(dp));
    assert(state != NULL);
    if (!TyArg_Parse(key, "s#", &krec.dptr, &tmp_size)) {
        return NULL;
    }
    ...
    if ( dbm_error(dp->di_dbm) ) {
        ...
        TyErr_SetString(state->dbm_error, "");
        return NULL;
    }
    return TyBytes_FromStringAndSize(drec.dptr, drec.dsize);
}

static int
dbm_ass_sub(TyObject *self, TyObject *v, TyObject *w)
{
    datum krec, drec;
    Ty_ssize_t tmp_size;
    dbmobject *dp = dbmobject_CAST(self);

    if ( !TyArg_Parse(v, "s#", &krec.dptr, &tmp_size) ) {
    ...
    if (w == NULL) {
        if ( dbm_delete(dp->di_dbm, krec) < 0 ) {
            ...
            if (dp->flags & O_RDWR) {
                ...
                TyErr_SetString(state->dbm_error, "cannot delete item from database");
            }
            return -1;
        }
    } else {
        if ( !TyArg_Parse(w, "s#", &drec.dptr, &tmp_size) ) {
            TyErr_SetString(TyExc_TypeError,
...
/*[clinic end generated code: output=f2a593b3038e5996 input=d3706a28fc051097]*/
{
    ...
    _dbm_state *state = TyType_GetModuleState(cls);
    assert(state != NULL);
...
static int
dbm_contains(TyObject *self, TyObject *arg)
{
    dbmobject *dp = dbmobject_CAST(self);
    datum key, val;
    Ty_ssize_t size;
...
/*[clinic end generated code: output=b4e55f8b6d482bc4 input=66b993b8349fa8c1]*/
{
    ...
    if (val.dptr != NULL) {
        return TyBytes_FromStringAndSize(val.dptr, val.dsize);
    }
...
/*[clinic end generated code: output=9c2f6ea6d0fb576c input=126a3ff15c5f8232]*/
{
    ...
    if (default_value == NULL) {
        default_value = TyBytes_FromStringAndSize(NULL, 0);
        if (default_value == NULL) {
            return NULL;
        }
        val.dptr = NULL;
        ...
        if ( !TyArg_Parse(default_value, "s#", &val.dptr, &tmp_size) ) {
            TyErr_SetString(TyExc_TypeError,
                "dbm mappings have bytes or string elements only");
            return NULL;
        }
        val.dsize = tmp_size;
        Ty_INCREF(default_value);
    }
    if (dbm_store(self->di_dbm, dbm_key, val, DBM_INSERT) < 0) {
        dbm_clearerr(self->di_dbm);
        TyErr_SetString(state->dbm_error, "cannot add item to database");
        Ty_DECREF(default_value);
        return NULL;
    }
...
static TyMethodDef dbm_methods[] = {
...
static TyType_Slot dbmtype_spec_slots[] = {
    {Ty_tp_dealloc, dbm_dealloc},
    {Ty_tp_traverse, dbm_traverse},
    {Ty_tp_methods, dbm_methods},
    {Ty_sq_contains, dbm_contains},
    {Ty_mp_length, dbm_length},
    {Ty_mp_subscript, dbm_subscript},
    {Ty_mp_ass_subscript, dbm_ass_sub},
    {Ty_nb_bool, dbm_bool},
    {0, 0}
};


static TyType_Spec dbmtype_spec = {
    .name = "_dbm.dbm",
...
/*[clinic end generated code: output=9527750f5df90764 input=d8cf50a9f81218c8]*/
{
    ...
    TyObject *self = newdbmobject(state, name, iflags, mode);
    Ty_DECREF(filenamebytes);
    return self;
}

static TyMethodDef dbmmodule_methods[] = {
    DBMOPEN_METHODDEF
    { 0, 0 },
};

static int
_dbm_exec(TyObject *module)
{
    _dbm_state *state = get_dbm_state(module);
    state->dbm_type = (TyTypeObject *)TyType_FromModuleAndSpec(module,
                                                        &dbmtype_spec, NULL);
    if (state->dbm_type == NULL) {
        return -1;
    }
    state->dbm_error = TyErr_NewException("_dbm.error", TyExc_OSError, NULL);
    if (state->dbm_error == NULL) {
        return -1;
    }
    if (TyModule_AddStringConstant(module, "library", which_dbm) < 0) {
        return -1;
    }
    if (TyModule_AddType(module, (TyTypeObject *)state->dbm_error) < 0) {
        return -1;
    }
    return 0;
}
...
static int
_dbm_module_traverse(TyObject *module, visitproc visit, void *arg)
{
    _dbm_state *state = get_dbm_state(module);
    Ty_VISIT(state->dbm_error);
    Ty_VISIT(state->dbm_type);
    return 0;
}

static int
_dbm_module_clear(TyObject *module)
{
    _dbm_state *state = get_dbm_state(module);
    Ty_CLEAR(state->dbm_error);
    Ty_CLEAR(state->dbm_type);
    return 0;
}
...
{
    (void)_dbm_module_clear((TyObject *)module);
}

static PyModuleDef_Slot _dbmmodule_slots[] = {
    {Ty_mod_exec, _dbm_exec},
    {Ty_mod_multiple_interpreters, Ty_MOD_PER_INTERPRETER_GIL_SUPPORTED},
    {Ty_mod_gil, Ty_MOD_GIL_NOT_USED},
    {0, NULL}
};

static struct TyModuleDef _dbmmodule = {
