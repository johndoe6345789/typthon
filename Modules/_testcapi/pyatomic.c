...
static TyObject * \
test_atomic_add_##suffix(TyObject *self, TyObject *obj) { \
    dtype x = 0; \
    assert(_Ty_atomic_add_##suffix(&x, 1) == 0); \
    assert(x == 1); \
    assert(_Ty_atomic_add_##suffix(&x, 2) == 1); \
    assert(x == 3); \
    assert(_Ty_atomic_add_##suffix(&x, -2) == 3); \
    assert(x == 1); \
    assert(_Ty_atomic_add_##suffix(&x, -1) == 1); \
    assert(x == 0); \
    assert(_Ty_atomic_add_##suffix(&x, -1) == 0); \
    assert(x == (dtype)-1); \
    assert(_Ty_atomic_add_##suffix(&x, -2) == (dtype)-1); \
    assert(x == (dtype)-3); \
...
static TyObject * \
test_atomic_compare_exchange_##suffix(TyObject *self, TyObject *obj) { \
    dtype x = (dtype)0; \
    dtype y = (dtype)1; \
    dtype z = (dtype)2; \
    ...
    assert(x == 0); \
    assert(y == 0); \
    assert(_Ty_atomic_compare_exchange_##suffix(&x, &y, z) == 1); \
    assert(x == z); \
    assert(y == 0); \
    assert(_Ty_atomic_compare_exchange_##suffix(&x, &y, z) == 0); \
    assert(x == z); \
...
static TyObject * \
test_atomic_exchange_##suffix(TyObject *self, TyObject *obj) { \
    dtype x = (dtype)0; \
    dtype y = (dtype)1; \
    dtype z = (dtype)2; \
    assert(_Ty_atomic_exchange_##suffix(&x, y) == (dtype)0); \
    assert(x == (dtype)1); \
    assert(_Ty_atomic_exchange_##suffix(&x, z) == (dtype)1); \
    assert(x == (dtype)2); \
...
static TyObject * \
test_atomic_load_store_##suffix(TyObject *self, TyObject *obj) { \
    dtype x = (dtype)0; \
    dtype y = (dtype)1; \
    dtype z = (dtype)2; \
    assert(_Ty_atomic_load_##suffix(&x) == (dtype)0); \
    assert(x == (dtype)0); \
    _Ty_atomic_store_##suffix(&x, y); \
    assert(_Ty_atomic_load_##suffix(&x) == (dtype)1); \
    assert(x == (dtype)1); \
...
static TyObject * \
test_atomic_and_or_##suffix(TyObject *self, TyObject *obj) { \
    dtype x = (dtype)0; \
    dtype y = (dtype)1; \
    dtype z = (dtype)3; \
    assert(_Ty_atomic_or_##suffix(&x, z) == (dtype)0); \
    assert(x == (dtype)3); \
...
static TyObject *
test_atomic_fences(TyObject *self, TyObject *obj) {
...
static TyObject *
test_atomic_release_acquire(TyObject *self, TyObject *obj) {
    void *x = NULL;
    void *y = &y;
    assert(_Ty_atomic_load_ptr_acquire(&x) == NULL);
...
static TyObject *
test_atomic_load_store_int_release_acquire(TyObject *self, TyObject *obj) { \
    int x = 0;
    int y = 1;
    int z = 2;
    assert(_Ty_atomic_load_int_acquire(&x) == 0);
    _Ty_atomic_store_int_release(&x, y);
    assert(x == y);
    assert(_Ty_atomic_load_int_acquire(&x) == y);
...
int
_PyTestCapi_Init_PyAtomic(TyObject *mod)
{
