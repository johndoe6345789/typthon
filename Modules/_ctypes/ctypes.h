...
#           define Ty_FFI_COMPLEX_AVAILABLE __builtin_available(macOS 10.15, *)
#       else
#           define Ty_FFI_COMPLEX_AVAILABLE 1
#       endif
#   else
#       define Ty_FFI_COMPLEX_AVAILABLE 1
#   endif
#else
#   define Ty_FFI_COMPLEX_AVAILABLE 0
#endif
...
typedef struct {
    TyTypeObject *DictRemover_Type;
    TyTypeObject *PyCArg_Type;
    TyTypeObject *PyCField_Type;
    TyTypeObject *PyCThunk_Type;
    TyTypeObject *StructParam_Type;
    TyTypeObject *PyCType_Type;
    TyTypeObject *PyCStructType_Type;
    TyTypeObject *UnionType_Type;
    TyTypeObject *PyCPointerType_Type;
    TyTypeObject *PyCArrayType_Type;
    TyTypeObject *PyCSimpleType_Type;
    TyTypeObject *PyCFuncPtrType_Type;
    TyTypeObject *PyCData_Type;
    TyTypeObject *Struct_Type;
    TyTypeObject *Union_Type;
    TyTypeObject *PyCArray_Type;
    TyTypeObject *Simple_Type;
    TyTypeObject *PyCPointer_Type;
    TyTypeObject *PyCFuncPtr_Type;
#ifdef MS_WIN32
    TyTypeObject *PyComError_Type;
#endif
    /* a callable object used for unpickling:
       strong reference to _ctypes._unpickle() function */
    TyObject *_unpickle;
    TyObject *array_cache;
    TyObject *error_object_name;  // callproc.c
    TyObject *TyExc_ArgError;
    TyObject *swapped_suffix;
} ctypes_state;


extern struct TyModuleDef _ctypesmodule;
...
static inline ctypes_state *
get_module_state_by_class(TyTypeObject *cls)
{
    ctypes_state *state = (ctypes_state *)_TyType_GetModuleState(cls);
    assert(state != NULL);
    return state;
}

static inline ctypes_state *
get_module_state_by_def(TyTypeObject *cls)
{
    TyObject *mod = TyType_GetModuleByDef(cls, &_ctypesmodule);
    assert(mod != NULL);
    return get_module_state(mod);
}


extern TyType_Spec pyctype_type_spec;
extern TyType_Spec carg_spec;
extern TyType_Spec cfield_spec;
extern TyType_Spec cthunk_spec;
...
// SETFUNC: write content of the TyObject *value* to the location at *ptr*;
...
typedef TyObject *(* GETFUNC)(void *ptr, Ty_ssize_t size);
typedef TyObject *(* SETFUNC)(void *ptr, TyObject *value, Ty_ssize_t size);
typedef PyCArgObject *(* PARAMFUNC)(ctypes_state *st, CDataObject *obj);

/* A default buffer in CDataObject, which can be used for small C types.  If
this buffer is too small, TyMem_Malloc will be called to create a larger one,
...
struct tagCDataObject {
    ...
    Ty_ssize_t b_size;          /* size of memory block in bytes */
    Ty_ssize_t b_length;        /* number of references we need */
    Ty_ssize_t b_index;         /* index of this object into base's
                               b_object list */
    TyObject *b_objects;        /* dictionary of references we need to keep, or Ty_None */
    union value b_value;
};
...
#define CThunk_CheckExact(st, v)        Ty_IS_TYPE(v, st->PyCThunk_Type)
...
typedef struct {
    ...
    Ty_ssize_t b_index;         /* index of this object into base's
                                   b_object list */
    TyObject *b_objects;        /* list of references we need to keep */
    union value b_value;
...
extern int PyCStructUnionType_update_stginfo(TyObject *fields, TyObject *type, int isStruct);
extern int TyType_stginfo(TyTypeObject *self, Ty_ssize_t *psize, Ty_ssize_t *palign, Ty_ssize_t *plength);
extern int PyObject_stginfo(TyObject *self, Ty_ssize_t *psize, Ty_ssize_t *palign, Ty_ssize_t *plength);



#define CDataObject_CheckExact(st, v)  Ty_IS_TYPE((v), (st)->PyCData_Type)
#define CDataObject_Check(st, v)       PyObject_TypeCheck((v), (st)->PyCData_Type)
#define _CDataObject_HasExternalBuffer(v)  ((v)->b_ptr != (char *)&(v)->b_value)

#define PyCSimpleTypeObject_CheckExact(st, v)  Ty_IS_TYPE((v), (st)->PyCSimpleType_Type)
...
extern TyObject *PyCData_AtAddress(ctypes_state *st, TyObject *type, void *buf);
extern TyObject *PyCData_FromBytes(ctypes_state *st, TyObject *type, char *data, Ty_ssize_t length);
...
extern TyMethodDef _ctypes_module_methods[];
...
typedef struct CFieldObject {
    ...
    Ty_ssize_t index;                   /* Index into CDataObject's object array */
    TyObject *proto;                    /* underlying ctype; must have StgInfo */
...
typedef struct {
    int initialized;
    Ty_ssize_t size;            /* number of bytes */
    Ty_ssize_t align;           /* alignment reqwuirements */
    Ty_ssize_t length;          /* number of fields */
    ffi_type ffi_type_pointer;
    TyObject *proto;            /* Only for Pointer/ArrayObject */
    SETFUNC setfunc;            /* Only for simple objects */
    GETFUNC getfunc;            /* Only for simple objects */
    PARAMFUNC paramfunc;

    /* Following fields only used by PyCFuncPtrType_Type instances */
    TyObject *argtypes;         /* tuple of CDataObjects */
    TyObject *converters;       /* tuple([t.from_param for t in argtypes]) */
    TyObject *restype;          /* CDataObject or NULL */
    TyObject *checker;
    TyObject *pointer_type;     /* __pointer_type__ attribute;
    ...
    /* pep3118 fields, pointers need TyMem_Free */
    char *format;
    int ndim;
    Ty_ssize_t *shape;
    /*      Ty_ssize_t *strides;    */ /* unused in ctypes */
    /*      Ty_ssize_t *suboffsets; */ /* unused in ctypes */
} StgInfo;
...
#define STGINFO_LOCK(stginfo)   Ty_BEGIN_CRITICAL_SECTION_MUT(&(stginfo)->mutex)
#define STGINFO_UNLOCK()        Ty_END_CRITICAL_SECTION()
...
{
    _Ty_CRITICAL_SECTION_ASSERT_MUTEX_LOCKED(&info->mutex);
...
struct tagPyCArgObject {
    ...
    TyObject *obj;
    Ty_ssize_t size; /* for the 'V' tag */
};

#define _PyCArgObject_CAST(op)  ((PyCArgObject *)(op))

#define PyCArg_CheckExact(st, v)        Ty_IS_TYPE(v, st->PyCArg_Type)
...
extern void _ctypes_extend_error(TyObject *exc_class, const char *fmt, ...);
...
extern ffi_type *_ctypes_get_ffi_type(ctypes_state *st, TyObject *obj);

extern void _ctypes_free_closure(void *);
extern void *_ctypes_alloc_closure(void);

extern TyObject *PyCData_FromBaseObj(ctypes_state *st, TyObject *type,
                                     TyObject *base, Ty_ssize_t index, char *adr);

extern int _ctypes_simple_instance(ctypes_state *st, TyObject *obj);

TyObject *_ctypes_get_errobj(ctypes_state *st, int **pspace);
...
#else
#define Ty_ffi_closure_free ffi_closure_free
#define Ty_ffi_closure_alloc ffi_closure_alloc
#endif
...
// Common helper:
static inline int
_stginfo_from_type(ctypes_state *state, TyTypeObject *type, StgInfo **result)
{
    *result = NULL;
    if (!PyObject_IsInstance((TyObject *)type, (TyObject *)state->PyCType_Type)) {
        // not a ctypes class.
        return 0;
    }
    StgInfo *info = PyObject_GetTypeData((TyObject *)type, state->PyCType_Type);
    assert(info != NULL);
    if (!info->initialized) {
...
// from a type:
static inline int
PyStgInfo_FromType(ctypes_state *state, TyObject *type, StgInfo **result)
{
    return _stginfo_from_type(state, (TyTypeObject *)type, result);
}
// from an instance:
static inline int
PyStgInfo_FromObject(ctypes_state *state, TyObject *obj, StgInfo **result)
{
    return _stginfo_from_type(state, Ty_TYPE(obj), result);
}
// from either a type or an instance:
static inline int
PyStgInfo_FromAny(ctypes_state *state, TyObject *obj, StgInfo **result)
{
    if (TyType_Check(obj)) {
        return _stginfo_from_type(state, (TyTypeObject *)obj, result);
    }
    return _stginfo_from_type(state, Ty_TYPE(obj), result);
}
...
_PyStgInfo_FromType_NoState(TyObject *type)
{
    TyTypeObject *PyCType_Type;
    if (TyType_GetBaseByToken(Ty_TYPE(type), &pyctype_type_spec, &PyCType_Type) < 0) {
        return NULL;
    }
    if (PyCType_Type == NULL) {
        TyErr_Format(TyExc_TypeError, "expected a ctypes type, got '%N'", type);
        return NULL;
    }

    StgInfo *info = PyObject_GetTypeData(type, PyCType_Type);
    Ty_DECREF(PyCType_Type);
    return info;
}

// Initialize StgInfo on a newly created type
static inline StgInfo *
PyStgInfo_Init(ctypes_state *state, TyTypeObject *type)
{
    if (!PyObject_IsInstance((TyObject *)type, (TyObject *)state->PyCType_Type)) {
    ...
    TyObject *module = TyType_GetModule(state->PyCType_Type);
    if (!module) {
        return NULL;
    }
    info->pointer_type = NULL;
    info->module = Ty_NewRef(module);
