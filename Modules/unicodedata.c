...
typedef struct {
    const unsigned char category;       /* index into
                                           _TyUnicode_CategoryNames */
    const unsigned char combining;      /* combining class value 0 - 255 */
    const unsigned char bidirectional;  /* index into
                                           _TyUnicode_BidirectionalNames */
    const unsigned char mirrored;       /* true if mirrored in bidir mode */
    const unsigned char east_asian_width;       /* index into
                                                   _TyUnicode_EastAsianWidth */
    const unsigned char normalization_quick_check; /* see is_normalized() */
} _TyUnicode_DatabaseRecord;
...
static const _TyUnicode_DatabaseRecord*
_getrecord_ex(Ty_UCS4 code)
{
    int index;
    if (code >= 0x110000)
        index = 0;
    ...
    return &_TyUnicode_Database_Records[index];
}
...
typedef struct previous_version {
    PyObject_HEAD
    const char *name;
    const change_record* (*getrecord)(Ty_UCS4);
    Ty_UCS4 (*normalization)(Ty_UCS4);
} PreviousDBVersion;
...
// TyModule_Check() is used to avoid having to retrieve the ucd_type.
// See unicodedata_functions comment to the rationale of this macro.
#define UCD_Check(self) (self != NULL && !TyModule_Check(self))

static TyObject*
new_previous_version(TyTypeObject *ucd_type,
                     const char*name, const change_record* (*getrecord)(Ty_UCS4),
                     Ty_UCS4 (*normalization)(Ty_UCS4))
{
...
/*[clinic end generated code: output=96e18c950171fd2f input=e27d6e4565cd29f2]*/
{
    long rc;
    Ty_UCS4 c = (Ty_UCS4)chr;
    rc = Ty_UNICODE_TODIGIT(c);
    if (rc < 0) {
        if (default_value == NULL) {
            TyErr_SetString(TyExc_ValueError, "not a digit");
            return NULL;
        }
        else {
            return Ty_NewRef(default_value);
...
/*[clinic end generated code: output=53ce281fe85b10c4 input=fdf5871a5542893c]*/
{
    ...
    if (!have_old)
        rc = Ty_UNICODE_TONUMERIC(c);
    if (rc == -1.0) {
        if (default_value == NULL) {
            TyErr_SetString(TyExc_ValueError, "not a numeric character");
            return NULL;
        }
        else {
            return Ty_NewRef(default_value);
...
/*[clinic end generated code: output=8571539ee2e6783a input=27d6f3d85050bc06]*/
{
    ...
    return TyUnicode_FromString(_TyUnicode_CategoryNames[index]);
}
...
/*[clinic end generated code: output=d36310ce2039bb92 input=b3d8f42cebfcf475]*/
{
    ...
    return TyUnicode_FromString(_TyUnicode_BidirectionalNames[index]);
}
...
/*[clinic end generated code: output=484e8537d9ee8197 input=c4854798aab026e0]*/
{
    ...
    return TyUnicode_FromString(_TyUnicode_EastAsianWidthNames[index]);
}
...
/*[clinic end generated code: output=7d699f3ec7565d27 input=e4c12459ad68507b]*/
{
    ...
    if (UCD_Check(self)) {
        const change_record *old = get_old_record(self, c);
        if (old->category_changed == 0)
            return Ty_GetConstant(Ty_CONSTANT_EMPTY_STR); /* unassigned */
    }
    ...
    assert(prefix_index < Ty_ARRAY_LENGTH(decomp_prefix));
    ...
    while (count-- > 0) {
        ...
        TyOS_snprintf(decomp + i, sizeof(decomp) - i, "%04X",
                      decomp_data[++index]);
        i += strlen(decomp + i);
    }
    return TyUnicode_FromStringAndSize(decomp, i);
}

static void
get_decomp_record(TyObject *self, Ty_UCS4 code,
...
static TyObject*
nfd_nfkd(TyObject *self, TyObject *input, int k)
{
    TyObject *result;
    Ty_UCS4 *output;
    Ty_ssize_t i, o, osize;
    int kind;
    const void *data;
    /* Longest decomposition in Unicode 3.2: U+FDFA */
    Ty_UCS4 stack[20];
    Ty_ssize_t space, isize;
    int index, prefix, count, stackptr;
    unsigned char prev, cur;

    stackptr = 0;
    isize = TyUnicode_GET_LENGTH(input);
    space = isize;
    ...
    osize = space;
    output = TyMem_NEW(Ty_UCS4, space);
    if (!output) {
        TyErr_NoMemory();
        return NULL;
    }
    ...
    while (i < isize) {
        stack[stackptr++] = TyUnicode_READ(kind, data, i++);
        while(stackptr) {
            Ty_UCS4 code = stack[--stackptr];
            ...
            if (space < 3) {
                ...
                osize += 10;
                space += 10;
                new_output = TyMem_Realloc(output, osize*sizeof(Ty_UCS4));
                if (new_output == NULL) {
                    TyMem_Free(output);
                    TyErr_NoMemory();
                    return NULL;
            ...
            if (UCD_Check(self)) {
                Ty_UCS4 value = ((PreviousDBVersion*)self)->normalization(code);
                if (value != 0) {
    ...
    kind = TyUnicode_KIND(result);
    data = TyUnicode_DATA(result);

    /* Sort canonically. */
    i = 0;
    prev = _getrecord_ex(TyUnicode_READ(kind, data, i))->combining;
    for (i++; i < TyUnicode_GET_LENGTH(result); i++) {
        cur = _getrecord_ex(TyUnicode_READ(kind, data, i))->combining;
        ...
        o = i - 1;
        while (1) {
            Ty_UCS4 tmp = TyUnicode_READ(kind, data, o+1);
            TyUnicode_WRITE(kind, data, o+1,
                            TyUnicode_READ(kind, data, o));
            TyUnicode_WRITE(kind, data, o, tmp);
            o--;
            if (o < 0)
                break;
            prev = _getrecord_ex(TyUnicode_READ(kind, data, o))->combining;
            if (prev == 0 || prev <= cur)
                break;
        }
        prev = _getrecord_ex(TyUnicode_READ(kind, data, i))->combining;
...
static int
find_nfc_index(const struct reindex* nfc, Ty_UCS4 code)
{
    unsigned int index;
...
static TyObject*
nfc_nfkc(TyObject *self, TyObject *input, int k)
{
    TyObject *result;
    int kind;
    const void *data;
    Ty_UCS4 *output;
    Ty_ssize_t i, i1, o, len;
    int f,l,index,index1,comb;
    Ty_UCS4 code;
    Ty_ssize_t skipped[20];
    int cskipped = 0;
  ...
  again:
    while (i < len) {
      ...
      code = TyUnicode_READ(kind, data, i);
      if (LBase <= code && code < (LBase+LCount) &&
          i + 1 < len &&
          VBase <= TyUnicode_READ(kind, data, i+1) &&
          TyUnicode_READ(kind, data, i+1) < (VBase+VCount)) {
          /* check L character is a modern leading consonant (0x1100 ~ 0x1112)
             and V character is a modern vowel (0x1161 ~ 0x1175). */
          int LIndex, VIndex;
          LIndex = code - LBase;
          VIndex = TyUnicode_READ(kind, data, i+1) - VBase;
          code = SBase + (LIndex*VCount+VIndex)*TCount;
          i+=2;
          if (i < len &&
              TBase < TyUnicode_READ(kind, data, i) &&
              TyUnicode_READ(kind, data, i) < (TBase+TCount)) {
              /* check T character is a modern trailing consonant
                 (0x11A8 ~ 0x11C2). */
              code += TyUnicode_READ(kind, data, i)-TBase;
              i++;
          }
          output[o++] = code;
          continue;
      ...
      output[o] = TyUnicode_READ(kind, data, i);
      while (i1 < len) {
          Ty_UCS4 code1 = TyUnicode_READ(kind, data, i1);
...
is_normalized_quickcheck(TyObject *self, TyObject *input, bool nfc, bool k,
                         bool yes_only)
{
    ...
    while (i < len) {
        ...
        const _TyUnicode_DatabaseRecord *record = _getrecord_ex(ch);
...
/*[clinic end generated code: output=11e5a3694e723ca5 input=a544f14cea79e508]*/
{
    ...
    if (TyUnicode_CompareWithASCIIString(form, "NFC") == 0) {
        nfc = true;
    }
    else if (TyUnicode_CompareWithASCIIString(form, "NFKC") == 0) {
        nfc = true;
        k = true;
    }
    else if (TyUnicode_CompareWithASCIIString(form, "NFD") == 0) {
        /* matches default values for `nfc` and `k` */
    }
    else if (TyUnicode_CompareWithASCIIString(form, "NFKD") == 0) {
        k = true;
    }
    else {
        TyErr_SetString(TyExc_ValueError, "invalid normalization form");
        return NULL;
    }
    ...
    if (m == MAYBE) {
        ...
        match = TyUnicode_Compare(input, cmp);
        Ty_DECREF(cmp);
        result = (match == 0) ? Ty_True : Ty_False;
    }
    else {
        result = (m == YES) ? Ty_True : Ty_False;
    }
...
/*[clinic end generated code: output=05ca4385a2ad6983 input=3a5206c0ad2833fb]*/
{
    ...
    if (TyUnicode_CompareWithASCIIString(form, "NFKC") == 0) {
        if (is_normalized_quickcheck(self, input,
                                     true,  true,  true) == YES) {
            return TyUnicode_FromObject(input);
        }
    ...
    if (TyUnicode_CompareWithASCIIString(form, "NFD") == 0) {
        if (is_normalized_quickcheck(self, input,
    ...
    if (TyUnicode_CompareWithASCIIString(form, "NFKD") == 0) {
        if (is_normalized_quickcheck(self, input,
                                     false, true,  true) == YES) {
            return TyUnicode_FromObject(input);
        }
...
static int
is_unified_ideograph(Ty_UCS4 code)
{
    return
                 ...
                 unsigned int label_offset, unsigned int namepos)
{
    ...
    for (unsigned int i = 0; i < size; i++) {
        if (packed_name_dawg[label_offset + i] != Ty_TOUPPER(name[namepos + i])) {
            if (i > 0) {
...
static int
_getucname(TyObject *self,
           Ty_UCS4 code, char* buffer, int buflen, int with_alias_and_seq)
{
    ...
    assert(buflen >= 0);
    return _inverse_dawg_lookup(buffer, Ty_SAFE_DOWNCAST(buflen, int, unsigned int), offset);
}

static int
capi_getucname(Ty_UCS4 code,
...
find_syllable(const char *str, int *len, int *pos, int count, int column)
{
    ...
    *len = -1;
    for (i = 0; i < count; i++) {
        ...
        len1 = Ty_SAFE_DOWNCAST(strlen(s), size_t, int);
        if (len1 <= *len)
            continue;
...
static int
_check_alias_and_seq(Ty_UCS4* code, int with_named_seq)
{
...
static int
_getcode(const char* name, int namelen, Ty_UCS4* code)
{
    ...
    assert(namelen >= 0);
    int position = _lookup_dawg_packed(name, Ty_SAFE_DOWNCAST(namelen, int, unsigned int));
    if (position < 0) {
        return 0;
    }
...
static int
capi_getcode(const char* name, int namelen, Ty_UCS4* code,
             int with_named_seq)
{
...
static void
unicodedata_destroy_capi(TyObject *capsule)
{
...
{
    _TyUnicode_Name_CAPI *capi = TyMem_Malloc(sizeof(_TyUnicode_Name_CAPI));
    if (capi == NULL) {
        TyErr_NoMemory();
        return NULL;
    }
...
/*[clinic end generated code: output=6bbb37a326407707 input=3e0367f534de56d9]*/
{
    ...
    if (!_getucname(self, c, name, NAME_MAXLEN, 0)) {
        if (default_value == NULL) {
            TyErr_SetString(TyExc_ValueError, "no such name");
            return NULL;
        }
        else {
            return Ty_NewRef(default_value);
...
/*[clinic end generated code: output=7f03fc4959b242f6 input=a557be0f8607a0d6]*/
{
    ...
    if (name_length > NAME_MAXLEN) {
        TyErr_SetString(TyExc_KeyError, "name too long");
        return NULL;
    }

    if (!_getcode(name, (int)name_length, &code)) {
        TyErr_Format(TyExc_KeyError, "undefined character name '%s'", name);
        return NULL;
    }
    if (UCD_Check(self)) {
        /* in 3.2.0 there are no aliases and named sequences */
    ...
    if (IS_NAMED_SEQ(code)) {
        ...
        return TyUnicode_FromKindAndData(TyUnicode_2BYTE_KIND,
    ...
    return TyUnicode_FromOrdinal(code);
}
...
// an UCD instance.
static TyMethodDef unicodedata_functions[] = {
...
static int
ucd_traverse(TyObject *self, visitproc visit, void *arg)
{
...
static TyType_Slot ucd_type_slots[] = {
    {Ty_tp_dealloc, ucd_dealloc},
    {Ty_tp_traverse, ucd_traverse},
    {Ty_tp_getattro, PyObject_GenericGetAttr},
    {Ty_tp_methods, unicodedata_functions},
    {Ty_tp_members, DB_members},
    {0, 0}
};

static TyType_Spec ucd_type_spec = {
...
static int
unicodedata_exec(TyObject *module)
{
    if (TyModule_AddStringConstant(module, "unidata_version", UNIDATA_VERSION) < 0) {
        return -1;
    }

    TyTypeObject *ucd_type = (TyTypeObject *)TyType_FromSpec(&ucd_type_spec);
    if (ucd_type == NULL) {
        return -1;
    }

    if (TyModule_AddType(module, ucd_type) < 0) {
        Ty_DECREF(ucd_type);
        return -1;
    }
    ...
    Ty_DECREF(ucd_type);
    if (TyModule_Add(module, "ucd_3_2_0", v) < 0) {
        return -1;
    }

    /* Export C API */
    if (TyModule_Add(module, "_ucnhash_CAPI", unicodedata_create_capi()) < 0) {
        return -1;
    }
    return 0;
}
...
static PyModuleDef_Slot unicodedata_slots[] = {
    {Ty_mod_exec, unicodedata_exec},
    {Ty_mod_multiple_interpreters, Ty_MOD_PER_INTERPRETER_GIL_SUPPORTED},
    {Ty_mod_gil, Ty_MOD_GIL_NOT_USED},
    {0, NULL}
};

static struct TyModuleDef unicodedata_module = {
