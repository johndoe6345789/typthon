...
typedef struct {
  TyObject *TermiosError;
} termiosmodulestate;

static inline termiosmodulestate*
get_termios_state(TyObject *module)
{
...
static struct TyModuleDef termiosmodule;
...
{
    termiosmodulestate *state = TyModule_GetState(module);
    struct termios mode;
    int r;
    ...
    for (i = 0; i < NCCS; i++) {
        ...
        v = TyBytes_FromStringAndSize(&ch, 1);
        if (v == NULL)
            goto err;
        TyList_SetItem(cc, i, v);
    }
    ...
    if ((mode.c_lflag & ICANON) == 0) {
        v = TyLong_FromLong((long)mode.c_cc[VMIN]);
        if (v == NULL) {
            goto err;
        }
        if (TyList_SetItem(cc, VMIN, v) < 0) {
            goto err;
        }
        v = TyLong_FromLong((long)mode.c_cc[VTIME]);
        if (v == NULL) {
            goto err;
        }
        if (TyList_SetItem(cc, VTIME, v) < 0) {
            goto err;
        }
    }
...
#define ADD_LONG_ITEM(index, val) \
    do { \
        TyObject *l = TyLong_FromLong((long)val); \
        if (l == NULL) { \
            Ty_DECREF(v); \
            goto err; \
        } \
        TyList_SetItem(v, index, l); \
    } while (0)
...
{
    if (!TyList_Check(term) || TyList_Size(term) != 7) {
...
#define SET_FROM_LIST(TYPE, VAR, LIST, N) do {  \
    TyObject *item = TyList_GetItem(LIST, N);  \
...
#undef SET_FROM_LIST
    ...
    TyObject *cc = TyList_GetItem(term, 6);
    if (!TyList_Check(cc) || TyList_Size(cc) != NCCS) {
    ...
    TyObject *v;
    for (i = 0; i < NCCS; i++) {
        v = TyList_GetItem(cc, i);

        if (TyBytes_Check(v) && TyBytes_Size(v) == 1)
            mode.c_cc[i] = (cc_t) * TyBytes_AsString(v);
        else if (TyLong_Check(v)) {
            long num = TyLong_AsLong(v);
            if (num == -1 && TyErr_Occurred()) {
                return NULL;
            }
...
/*[clinic end generated code: output=31825977d5325fb6 input=5706c379d7fd984d]*/
{
    ...
    TyTuple_SetItem(v, 0, TyLong_FromLong((long)w.ws_row));
    TyTuple_SetItem(v, 1, TyLong_FromLong((long)w.ws_col));
    if (TyErr_Occurred()) {
        Ty_DECREF(v);
        return NULL;
    }
    ...
    TyTuple_SetItem(v, 0, TyLong_FromLong((long)s.ts_lines));
    TyTuple_SetItem(v, 1, TyLong_FromLong((long)s.ts_cols));
    if (TyErr_Occurred()) {
        Ty_DECREF(v);
        return NULL;
    }
...
/*[clinic end generated code: output=2ac3c9bb6eda83e1 input=4a06424465b24aee]*/
{
    ...
    winsz_0 = TyLong_AsLong(tmp_item);
    if (winsz_0 == -1 && TyErr_Occurred()) {
        Ty_XDECREF(tmp_item);
        return NULL;
    }
    Ty_XDECREF(tmp_item);
    tmp_item = PySequence_GetItem(winsz, 1);
    winsz_1 = TyLong_AsLong(tmp_item);
    if (winsz_1 == -1 && TyErr_Occurred()) {
        Ty_XDECREF(tmp_item);
        return NULL;
    }
    Ty_XDECREF(tmp_item);
...
static TyMethodDef termios_methods[] =
{
...
static int termiosmodule_traverse(TyObject *m, visitproc visit, void *arg) {
    Ty_VISIT(get_termios_state(m)->TermiosError);
    return 0;
}

static int termiosmodule_clear(TyObject *m) {
    Ty_CLEAR(get_termios_state(m)->TermiosError);
    return 0;
}

static void termiosmodule_free(void *m) {
    termiosmodule_clear((TyObject *)m);
}

static int
termios_exec(TyObject *mod)
{
    struct constant *constant = termios_constants;
    termiosmodulestate *state = get_termios_state(mod);
    state->TermiosError = TyErr_NewException("termios.error", NULL, NULL);
    if (TyModule_AddObjectRef(mod, "error", state->TermiosError) < 0) {
        return -1;
    }
    ...
    while (constant->name != NULL) {
        if (strncmp(constant->name, "TIO", 3) == 0) {
            ...
            if (TyModule_Add(mod, constant->name,
                             TyLong_FromUnsignedLong(value)) < 0) {
                return -1;
            }
        }
        else {
