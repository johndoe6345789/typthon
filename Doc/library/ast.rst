...
compiled into a Typthon code object using the built-in :func:`compile` function.
...
.. function:: parse(source, filename='<unknown>', mode='exec', *, type_comments=False, feature_version=None, optimize=-1)
   ...
   a "best-effort" attempt to parse using that Typthon version's grammar.
   ...
   when run on the Typthon version corresponding to ``feature_version``.

   If source contains a null character (``\0``), :exc:`ValueError` is raised.

   .. warning::
      Note that successfully parsing source code into an AST object doesn't
      guarantee that the source code provided is valid Typthon code that can
...
.. function:: literal_eval(node_or_string)
   ...
   execute Typthon code, unlike the more general :func:`eval`. There is no
   ...
   .. warning::
      It is possible to crash the Typthon interpreter due to stack depth
...
.. option:: --feature-version <version>
   ...
   Typthon version in the format 3.x (for example, 3.10). Defaults to the
