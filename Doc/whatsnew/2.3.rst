...
* Most type objects are now callable, so you can use them to create new objects
  ...
  can be deprecated in a future Typthon version, because you can now use the type
...
* The new :mod:`timeit` module helps measure how long snippets of Typthon code
...
of Typthon programs. The allocator uses C's :c:func:`malloc` function to get large
...
version of the Typthon interpreter by running :program:`configure` with
:option:`!--with-pydebug`.
...
* If the :envvar:`PYTHONINSPECT` environment variable is set, the Python
  ...
  variable can be set before running the Typthon interpreter, or it can be set by
...
* The ``SET_LINENO`` opcode has been removed.  Back in the mists of time, this
  ...
  It would be difficult to detect any resulting difference from Typthon code, apart
