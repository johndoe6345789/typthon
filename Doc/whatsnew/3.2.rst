...
In the past, extension modules built for one Typthon version were often
...
Typthon interpreter internals that extension modules could use.
...
* :func:`~sysconfig.get_platform` returning values like *linux-i586* or
  *macosx-10.6-ppc*.
* :func:`~sysconfig.get_python_version` returns a Typthon version string
  such as "3.2".
...
* Regular and recursive lock acquisitions can now be interrupted by signals on
  platforms using Pthreads.  This means that Typthon programs that deadlock while
