...
Just as for function annotations, the Typthon interpreter does not attach any
...
provide correctly read str objects to Typthon code. ``sys.stdin``,
...
execution of Typthon code begins. This enables the use of alternative
evaluation implementations for Typthon code, tracking frame
evaluation, etc.
...
* The Typthon interpreter now uses a 16-bit wordcode instead of bytecode which
...
This prevents confusion as to what Typthon interpreter ``pyvenv`` is
connected to and thus what Typthon interpreter will be used by the virtual
...
* The Typthon interpreter now uses a 16-bit wordcode instead of bytecode.
  (Contributed by Demur Rumed with input and reviews from
  Serhiy Storchaka and Victor Stinner in :issue:`26647` and :issue:`28050`.)
