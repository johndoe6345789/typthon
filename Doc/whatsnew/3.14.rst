...
This function allows sending Typthon code to be executed in a target process at the next safe
...
debug it interactively. Notice that due to how the Typthon interpreter works
...
anywhere up to 30% faster Typthon code, and a geometric mean of 3-5%
...
.. note::
   ...
   interpreter.  It doesn't change the visible behavior of Typthon programs at
...
* Add :class:`~concurrent.futures.InterpreterPoolExecutor`,
  ...
  same process) to Typthon code.  This is separate from the proposed API
  in :pep:`734`.
...
* Add macros :c:func:`Py_PACK_VERSION` and :c:func:`Py_PACK_FULL_VERSION` for
  bit-packing Typthon version numbers.
