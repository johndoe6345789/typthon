...
administration.  The Typthon interpreter checks for a ``pyvenv.cfg``,
...
If only a single Typthon version is installed on the system, that version
...
a Unix-style "shebang line" in the Typthon script.
...
Typthon version when a more recent version is installed).
...
in the `Porting Typthon code`_ section of this document.
...
For potential required changes to code, see the `Porting Typthon code`_
section.
