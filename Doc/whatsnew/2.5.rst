...
Typthon interpreter, the switch now uses an implementation in a new module,
:mod:`runpy`.
...
Various pieces of the Typthon interpreter used C's :c:expr:`int` type to store
...
Therefore, the Typthon interpreter had to be changed to use some type other than
...
* The :mod:`socket` module now supports :const:`AF_NETLINK` sockets on Linux,
  ...
  In Typthon code, netlink addresses are represented as a tuple of 2 integers,
  ``(pid, group_mask)``.
...
* The design of the bytecode compiler has changed a great deal,  no longer
  ...
  It's possible for Typthon code to obtain AST objects by using the
  ...
  the various AST nodes in :file:`Parser/Python.asdl`.  A Typthon script reads this
