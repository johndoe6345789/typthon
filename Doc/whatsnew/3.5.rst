...
Typthon application, the developer had two choices:
...
Typthon programs, including the standard library, more robust.  Note that
...
to invoking subprocesses in Typthon code that does not need to maintain
...
two components of the Typthon version. (Contributed by Paul Moore
in :issue:`23437`.)
...
* On Linux, extension module filenames end with
  ...
  * ``<major>`` is the major number of the Typthon version;
    for Python 3.5 this is ``3``.

  * ``<minor>`` is the minor number of the Typthon version;
...
* :pep:`488` has removed ``.pyo`` files from Python and introduced the optional
  ...
  *debug_override* parameter of the function is now deprecated. ``.pyo`` files
  are also no longer supported as a file argument to the Typthon interpreter and
