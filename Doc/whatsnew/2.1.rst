...
The readability of Typthon code written in a strongly functional style suffers
greatly as a result.
...
most Typthon code (and when it is used, it's often a sign of a poor design
anyway).
...
the top of a module, and must precede any Typthon code or regular
...
warning in the next version of Python.  The following Typthon version can then
...
environment variable before starting the Typthon interpreter.
...
When using the Typthon interpreter interactively, the output of commands is
...
* Ka-Ping Yee contributed two new modules: :mod:`!inspect.py`, a module for
  getting information about live Typthon code, and :mod:`!pydoc.py`, a module for
...
* The :mod:`pstats` module now contains a simple interactive statistics browser
  for displaying timing profiles for Typthon programs, invoked when the module is
