...
#include "pycore_code.h"            // _Ty_GetBaseCodeUnit
#include "pycore_function.h"        // _TyFunction_LookupByVersion()
#include "pycore_interpframe.h"
#include "pycore_object.h"          // _TyObject_GC_UNTRACK()
#include "pycore_opcode_metadata.h" // _TyOpcode_OpName[]
#include "pycore_opcode_utils.h"  // MAX_REAL_OPCODE
#include "pycore_optimizer.h"     // _Ty_uop_analyze_and_optimize()
...
static bool
has_space_for_executor(PyCodeObject *code, _Ty_CODEUNIT *instr)
{
...
static int32_t
get_index_for_executor(PyCodeObject *code, _Ty_CODEUNIT *instr)
{
    if (instr->op.code == ENTER_EXECUTOR) {
        return instr->op.arg;
    }
    ...
    if (size == capacity) {
        ...
        _PyExecutorArray *new = TyMem_Realloc(
            old,
...
static void
insert_executor(PyCodeObject *code, _Ty_CODEUNIT *instr, int index, _PyExecutorObject *executor)
{
    Ty_INCREF(executor);
    if (instr->op.code == ENTER_EXECUTOR) {
        assert(index == instr->op.arg);
        _Ty_ExecutorDetach(code->co_executors->executors[index]);
    }
    else {
    ...
    executor->vm_data.index = (int)(instr - _TyCode_CODE(code));
...
static int
uop_optimize(_PyInterpreterFrame *frame, _Ty_CODEUNIT *instr,
...
_PyOptimizer_Optimize(
    _PyInterpreterFrame *frame, _Ty_CODEUNIT *start,
    _PyExecutorObject **executor_ptr, int chain_depth)
{
    _PyStackRef *stack_pointer = frame->stackpointer;
    assert(_TyInterpreterState_GET()->jit);
    ...
    int err = uop_optimize(frame, start, executor_ptr, (int)(stack_pointer - _TyFrame_Stackbase(frame)), progress_needed);
    if (err <= 0) {
        return err;
    }
...
get_executor_lock_held(PyCodeObject *code, int offset)
{
    ...
    for (int i = 0 ; i < code_len;) {
        if (_TyCode_CODE(code)[i].op.code == ENTER_EXECUTOR && i*2 == offset) {
            int oparg = _TyCode_CODE(code)[i].op.arg;
    ...
    TyErr_SetString(TyExc_ValueError, "no executor at given byte offset");
    return NULL;
}

_PyExecutorObject *
_Ty_GetExecutor(PyCodeObject *code, int offset)
{
...
static int executor_clear(TyObject *executor);
...
void
_Ty_ClearExecutorDeletionList(TyInterpreterState *interp)
{
...
const char *
_TyUOpName(int index)
{
    if (index < 0 || index > MAX_UOP_ID) {
        return NULL;
    }
    return _TyOpcode_uop_name[index];
}
...
static TyObject *
uop_item(TyObject *op, Ty_ssize_t index)
{
    _PyExecutorObject *self = _PyExecutorObject_CAST(op);
    Ty_ssize_t len = uop_len(op);
    if (index < 0 || index >= len) {
        TyErr_SetNone(TyExc_IndexError);
        return NULL;
    }
    const char *name = _TyUOpName(self->trace[index].opcode);
    if (name == NULL) {
        name = "<nil>";
    }
    TyObject *oname = _TyUnicode_FromASCII(name, strlen(name));
    if (oname == NULL) {
        return NULL;
    }
    TyObject *oparg = TyLong_FromUnsignedLong(self->trace[index].oparg);
    if (oparg == NULL) {
        Ty_DECREF(oname);
        return NULL;
    }
    TyObject *target = TyLong_FromUnsignedLong(self->trace[index].target);
    if (oparg == NULL) {
        Ty_DECREF(oparg);
        Ty_DECREF(oname);
        return NULL;
    }
    TyObject *operand = TyLong_FromUnsignedLongLong(self->trace[index].operand0);
    if (operand == NULL) {
        Ty_DECREF(target);
        Ty_DECREF(oparg);
        Ty_DECREF(oname);
    ...
    TyObject *args[4] = { oname, oparg, target, operand };
    return _TyTuple_FromArraySteal(args, 4);
}
...
static int
executor_traverse(TyObject *o, visitproc visit, void *arg)
{
    _PyExecutorObject *executor = _PyExecutorObject_CAST(o);
    for (uint32_t i = 0; i < executor->exit_count; i++) {
        Ty_VISIT(executor->exits[i].executor);
    }
    return 0;
}
...
static TyMethodDef uop_executor_methods[] = {
...
static int
executor_is_gc(TyObject *o)
{
    return !_Ty_IsImmortal(o);
}

TyTypeObject _PyUOpExecutor_Type = {
    TyVarObject_HEAD_INIT(&TyType_Type, 0)
    .tp_name = "uop_executor",
    .tp_basicsize = offsetof(_PyExecutorObject, exits),
    .tp_itemsize = 1,
    .tp_flags = Ty_TPFLAGS_DEFAULT | Ty_TPFLAGS_DISALLOW_INSTANTIATION | Ty_TPFLAGS_HAVE_GC,
...
#define INSTR_IP(INSTR, CODE) \
    ((uint32_t)((INSTR) - ((_Ty_CODEUNIT *)(CODE)->co_code_adaptive)))
...
#define RESERVE(needed) RESERVE_RAW((needed) + 3, _TyUOpName(opcode))
...
#define TRACE_STACK_PUSH() \
    ...
    assert(func == NULL || func->func_code == (TyObject *)code); \
...
#define TRACE_STACK_POP() \
    if (trace_stack_depth <= 0) { \
        Ty_FatalError("Trace stack underflow\n"); \
    } \
    trace_stack_depth--; \
    ...
    _PyBloomFilter *dependencies, bool progress_needed)
{
    ...
    _Ty_BloomFilter_Add(dependencies, initial_code);
    _Ty_CODEUNIT *initial_instr = instr;
    int trace_length = 0;
    ...
    DPRINTF(2,
            ...
            TyUnicode_AsUTF8(code->co_qualname),
            TyUnicode_AsUTF8(code->co_filename),
            code->co_firstlineno,
    ...
    for (;;) {
        ...
        DPRINTF(2, "%d: %s(%d)\n", target, _TyOpcode_OpName[opcode], oparg);
        ...
        switch (opcode) {
            ...
            case POP_JUMP_IF_TRUE:
            {
                ...
                int bitcount = _Ty_popcount32(counter);
                ...
                DPRINTF(2, "%d: %s(%d): counter=%04x, bitcount=%d, likely=%d, confidence=%d, uopcode=%s\n",
                        target, _TyOpcode_OpName[opcode], oparg,
                        counter, bitcount, jump_likely, confidence, _TyUOpName(uopcode));
                if (confidence < CONFIDENCE_CUTOFF) {
                    DPRINTF(2, "Confidence too low (%d < %d)\n", confidence, CONFIDENCE_CUTOFF);
                    OPT_STAT_INC(low_confidence);
                    goto done;
                }
                _Ty_CODEUNIT *next_instr = instr + 1 + _TyOpcode_Caches[_TyOpcode_Deopt[opcode]];
                _Ty_CODEUNIT *target_instr = next_instr + oparg;
                if (jump_likely) {
            ...
            {
                instr += 1 + _TyOpcode_Caches[_TyOpcode_Deopt[opcode]] - (int)oparg;
                if (jump_seen) {
            ...
            default:
            {
                const struct opcode_macro_expansion *expansion = &_TyOpcode_macro_expansion[opcode];
                if (expansion->nuops > 0) {
                    ...
                    for (int i = 0; i < nuops; i++) {
                        ...
                        switch (expansion->uops[i].size) {
                            ...
                            case OPARG_REPLACED:
...
#ifdef Ty_DEBUG
                                {
                                    ...
                                    assert(_Ty_GetBaseCodeUnit(code, jump_target).op.code == END_FOR);
                                    assert(_Ty_GetBaseCodeUnit(code, jump_target+1).op.code == POP_ITER);
                                }
#endif
                                break;
                            case OPERAND1_1:
                            ...
                            default:
                                ...
                                Ty_FatalError("garbled expansion");
                        }
                        ...
                        if (uop == _PUSH_FRAME) {
                            ...
                            assert(_TyOpcode_Deopt[opcode] == CALL || _TyOpcode_Deopt[opcode] == CALL_KW);
                            int func_version_offset =
                                offsetof(_PyCallCache, func_version)/sizeof(_Ty_CODEUNIT)
                                // Add one to account for the actual opcode/oparg pair:
                                + 1;
                            uint32_t func_version = read_u32(&instr[func_version_offset].cache);
                            PyCodeObject *new_code = NULL;
                            PyFunctionObject *new_func =
                                _TyFunction_LookupByVersion(func_version, (TyObject **) &new_code);
                            ...
                            if (new_code != NULL) {
                                if (new_code == code) {
                                    ...
                                    DPRINTF(2, "Bailing on recursive call to %s (%s:%d)\n",
                                            TyUnicode_AsUTF8(new_code->co_qualname),
                                            TyUnicode_AsUTF8(new_code->co_filename),
                                ...
                                instr += _TyOpcode_Caches[_TyOpcode_Deopt[opcode]] + 1;
                                TRACE_STACK_PUSH();
                                _Ty_BloomFilter_Add(dependencies, new_code);
        ...
        instr += _TyOpcode_Caches[_TyOpcode_Deopt[opcode]];
...
sanity_check(_PyExecutorObject *executor)
{
    ...
    for (; i < executor->code_size; i++) {
        ...
        CHECK(_TyOpcode_uop_name[opcode] != NULL);
        switch(inst->format) {
            case UOP_FORMAT_TARGET:
...
make_executor_from_uops(_PyUOpInstruction *buffer, int length, const _PyBloomFilter *dependencies)
{
    ...
    _Ty_ExecutorInit(executor, dependencies);
#ifdef Ty_DEBUG
    char *python_lltrace = Ty_GETENV("PYTHON_LLTRACE");
    int lltrace = 0;
    if (python_lltrace != NULL && *python_lltrace >= '0') {
...
int effective_trace_length(_PyUOpInstruction *buffer, int length)
{
    ...
    Ty_FatalError("No terminating instruction");
    Ty_UNREACHABLE();
}
#endif
    ...
    bool progress_needed)
{
    ...
    _Ty_BloomFilter_Init(&dependencies);
    ...
    char *env_var = Ty_GETENV("PYTHON_UOPS_OPTIMIZE");
    if (env_var == NULL || *env_var == '\0' || *env_var > '0') {
        length = _Ty_uop_analyze_and_optimize(frame, buffer,
                                           length,
    ...
    for (int pc = 0; pc < length; pc++) {
        ...
        if (oparg < _PyUop_Replication[opcode]) {
            ...
            assert(strncmp(_TyOpcode_uop_name[buffer[pc].opcode], _TyOpcode_uop_name[opcode], strlen(_TyOpcode_uop_name[opcode])) == 0);
        }
        else if (is_terminator(&buffer[pc])) {
            break;
        }
        assert(_TyOpcode_uop_name[buffer[pc].opcode]);
    }
...
void
_Ty_BloomFilter_Init(_PyBloomFilter *bloom)
{
    for (int i = 0; i < _Ty_BLOOM_FILTER_WORDS; i++) {
        bloom->bits[i] = 0;
    }
}
 ...
 * the 64bit hash. */
void
_Ty_BloomFilter_Add(_PyBloomFilter *bloom, void *ptr)
{
...
void
_Ty_ExecutorInit(_PyExecutorObject *executor, const _PyBloomFilter *dependency_set)
{
...
void
_Ty_ExecutorDetach(_PyExecutorObject *executor)
{
    PyCodeObject *code = executor->vm_data.code;
    if (code == NULL) {
        return;
    }
    _Ty_CODEUNIT *instruction = &_TyCode_CODE(code)[executor->vm_data.index];
...
executor_clear(TyObject *op)
{
    ...
    for (uint32_t i = 0; i < executor->exit_count; i++) {
        ...
        Ty_CLEAR(executor->exits[i].executor);
    }
    _Ty_ExecutorDetach(executor);
    Ty_DECREF(executor);
    return 0;
}

void
_Ty_Executor_DependsOn(_PyExecutorObject *executor, void *obj)
{
    assert(executor->vm_data.valid);
    _Ty_BloomFilter_Add(&executor->vm_data.bloom, obj);
}
 ...
 */
void
_Ty_Executors_InvalidateDependency(TyInterpreterState *interp, void *obj, int is_invalidation)
{
    _PyBloomFilter obj_filter;
    _Ty_BloomFilter_Init(&obj_filter);
    _Ty_BloomFilter_Add(&obj_filter, obj);
    /* Walk the list of executors */
    /* TO DO -- Use a tree to avoid traversing as many objects */
    TyObject *invalidate = TyList_New(0);
    if (invalidate == NULL) {
        goto error;
    }
    ...
    for (_PyExecutorObject *exec = interp->executor_list_head; exec != NULL;) {
        ...
        if (bloom_filter_may_contain(&exec->vm_data.bloom, &obj_filter) &&
            TyList_Append(invalidate, (TyObject *)exec))
        {
            goto error;
        }
        exec = next;
    }
    for (Ty_ssize_t i = 0; i < TyList_GET_SIZE(invalidate); i++) {
        TyObject *exec = TyList_GET_ITEM(invalidate, i);
        executor_clear(exec);
        if (is_invalidation) {
            OPT_STAT_INC(executors_invalidated);
        }
    }
    Ty_DECREF(invalidate);
    return;
error:
    TyErr_Clear();
    Ty_XDECREF(invalidate);
    // If we're truly out of memory, wiping out everything is a fine fallback:
    _Ty_Executors_InvalidateAll(interp, is_invalidation);
}

/* Invalidate all executors */
void
_Ty_Executors_InvalidateAll(TyInterpreterState *interp, int is_invalidation)
{
    while (interp->executor_list_head) {
        ...
        if (executor->vm_data.code) {
            ...
            _TyCode_Clear_Executors(executor->vm_data.code);
        }
        else {
            executor_clear((TyObject *)executor);
        }
        if (is_invalidation) {
            OPT_STAT_INC(executors_invalidated);
        }
...
void
_Ty_Executors_InvalidateCold(TyInterpreterState *interp)
{
    /* Walk the list of executors */
    /* TO DO -- Use a tree to avoid traversing as many objects */
    TyObject *invalidate = TyList_New(0);
    ...
    for (_PyExecutorObject *exec = interp->executor_list_head; exec != NULL;) {
        ...
        if (!exec->vm_data.warm && TyList_Append(invalidate, (TyObject *)exec) < 0) {
            goto error;
        }
        else {
...
static void
write_str(TyObject *str, FILE *out)
{
    // Encode the Unicode object to the specified encoding
    TyObject *encoded_obj = TyUnicode_AsEncodedString(str, "utf8", "strict");
    if (encoded_obj == NULL) {
        TyErr_Clear();
        return;
    }
    const char *encoded_str = TyBytes_AsString(encoded_obj);
    Ty_ssize_t encoded_size = TyBytes_Size(encoded_obj);
    fwrite(encoded_str, 1, encoded_size, out);
    Ty_DECREF(encoded_obj);
}
...
find_line_number(PyCodeObject *code, _PyExecutorObject *executor)
{
    ...
    for (int i = 0; i < code_len; i++) {
        ...
        if (opcode == ENTER_EXECUTOR) {
            ...
            if (exec == executor) {
                return TyCode_Addr2Line(code, i*2);
            }
        }
        i += _TyOpcode_Caches[_Ty_GetBaseCodeUnit(code, i).op.code];
    }
    return -1;
}
 ...
 * If Ty_STATS is enabled, execution counts are included.
 *
...
executor_to_gv(_PyExecutorObject *executor, FILE *out)
{
    ...
    for (uint32_t i = 0; i < executor->code_size; i++) {
        ...
        const char *opname = _TyOpcode_uop_name[inst->opcode];
#ifdef Ty_STATS
...
_PyDumpExecutors(FILE *out)
{
    TyErr_SetString(TyExc_NotImplementedError, "No JIT available");
    return -1;
}
