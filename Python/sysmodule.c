...
#include "pycore_ceval.h"         // _TyEval_SetAsyncGenFinalizer()
#include "pycore_frame.h"         // _PyInterpreterFrame
#include "pycore_import.h"        // _TyImport_SetDLOpenFlags()
#include "pycore_initconfig.h"    // _TyStatus_EXCEPTION()
#include "pycore_interpframe.h"   // _TyFrame_GetFirstComplete()
#include "pycore_long.h"          // _PY_LONG_MAX_STR_DIGITS_THRESHOLD
#include "pycore_modsupport.h"    // _TyModule_CreateInitialized()
#include "pycore_namespace.h"     // _PyNamespace_New()
#include "pycore_object.h"        // _TyObject_DebugTypeStats()
...
#include "pycore_pylifecycle.h"   // _TyErr_WriteUnraisableDefaultHook()
#include "pycore_pymath.h"        // _PY_SHORT_FLOAT_REPR
#include "pycore_pymem.h"         // _TyMem_DefaultRawFree()
#include "pycore_pystate.h"       // _TyThreadState_GET()
#include "pycore_pystats.h"       // _Ty_PrintSpecializationStats()
#include "pycore_structseq.h"     // _PyStructSequence_InitBuiltinWithFlags()
#include "pycore_sysmodule.h"     // export _TySys_GetSizeOf()
#include "pycore_unicodeobject.h" // _TyUnicode_InternImmortal()

#include "pydtrace.h"             // PyDTrace_AUDIT()
#include "osdefs.h"               // DELIM
#include "stdlib_module_names.h"  // _Ty_stdlib_module_names
...
TyObject *
_TySys_GetRequiredAttr(TyObject *name)
{
    ...
    TyObject *value;
    if (TyDict_GetItemRef(sysdict, name, &value) == 0) {
        TyErr_Format(TyExc_RuntimeError, "lost sys.%U", name);
    }
    return value;
}
...
_TySys_GetRequiredAttrString(const char *name)
{
    ...
    TyObject *value;
    if (TyDict_GetItemStringRef(sysdict, name, &value) == 0) {
        TyErr_Format(TyExc_RuntimeError, "lost sys.%s", name);
    }
    return value;
}

int
_TySys_GetOptionalAttr(TyObject *name, TyObject **value)
{
    if (!TyUnicode_Check(name)) {
        TyErr_Format(TyExc_TypeError,
                     "attribute name must be string, not '%.200s'",
    ...
    return TyDict_GetItemRef(sysdict, name, value);
}
...
_TySys_GetOptionalAttrString(const char *name, TyObject **value)
{
    ...
    return TyDict_GetItemStringRef(sysdict, name, value);
}
...
TySys_GetObject(const char *name)
{
    ...
    TyObject *value;
    (void) TyDict_GetItemStringRef(sysdict, name, &value);
    /* XXX Suppress a new exception if it was raised and restore
     * the old one. */
    if (_TyErr_Occurred(tstate)) {
        TyErr_FormatUnraisable("Exception ignored in TySys_GetObject()");
    }
    _TyErr_SetRaisedException(tstate, exc);
    Ty_XDECREF(value);  // return a borrowed reference
    return value;
}
...
sys_set_object(TyInterpreterState *interp, TyObject *key, TyObject *v)
{
    ...
    TyObject *sd = interp->sysdict;
    if (sd == NULL) {
        TyErr_SetString(TyExc_RuntimeError, "no sys module");
        return -1;
    }
    if (v == NULL) {
        if (TyDict_Pop(sd, key, NULL) < 0) {
            return -1;
        }
        return 0;
    }
    else {
        return TyDict_SetItem(sd, key, v);
    }
}
...
{
    if (verbose) {
        TySys_WriteStderr("# clear sys.%s\n", name);
    }
    /* To play it safe, we set the attr to None instead of deleting it. */
    if (TyDict_SetItemString(interp->sysdict, name, Ty_None) < 0) {
        return -1;
    }
    return 0;
}
                 ...
                 const char *argFormat, va_list vargs)
{
    ...
    TyObject *eventName = NULL;
    TyObject *eventArgs = NULL;
    TyObject *hooks = NULL;
    TyObject *hook = NULL;
    int res = -1;
    ...
    TyObject *exc = _TyErr_GetRaisedException(ts);

    /* Initialize event args now */
    if (argFormat && argFormat[0]) {
        eventArgs = Ty_VaBuildValue(argFormat, vargs);
        if (eventArgs && !TyTuple_Check(eventArgs)) {
            TyObject *argTuple = TyTuple_Pack(1, eventArgs);
            Ty_SETREF(eventArgs, argTuple);
        }
    }
    else {
        eventArgs = TyTuple_New(0);
    }
    if (!eventArgs) {
        goto exit;
    }
    ...
    _Ty_AuditHookEntry *e = is->runtime->audit_hooks.head;
    for (; e; e = e->next) {
    ...
    if (is->audit_hooks) {
        eventName = TyUnicode_FromString(event);
        if (!eventName) {
            goto exit;
        }
        ...
        TyThreadState_EnterTracing(ts);
        while ((hook = TyIter_Next(hooks)) != NULL) {
            TyObject *o;
            int canTrace = PyObject_GetOptionalAttr(hook, &_Ty_ID(__cantrace__), &o);
            if (o) {
            ...
            if (canTrace) {
                TyThreadState_LeaveTracing(ts);
            }
            TyObject* args[2] = {eventName, eventArgs};
            o = _TyObject_VectorcallTstate(ts, hook, args, 2, NULL);
            if (canTrace) {
                TyThreadState_EnterTracing(ts);
            }
            if (!o) {
...
exit:
    Ty_XDECREF(hook);
    Ty_XDECREF(hooks);
    Ty_XDECREF(eventName);
    Ty_XDECREF(eventArgs);

    if (!res) {
        _TyErr_SetRaisedException(ts, exc);
    }
    else {
        assert(_TyErr_Occurred(ts));
        Ty_XDECREF(exc);
    }
...
int
TySys_AuditTuple(const char *event, TyObject *args)
{
    if (args == NULL) {
        return TySys_Audit(event, NULL);
    }

    if (!TyTuple_Check(args)) {
        TyErr_Format(TyExc_TypeError, "args must be tuple, got %s",
                     Ty_TYPE(args)->tp_name);
        return -1;
    }
    return TySys_Audit(event, "O", args);
}
...
_TySys_ClearAuditHooks(TyThreadState *ts)
{
    ...
    _TySys_Audit(ts, "cpython._TySys_ClearAuditHooks", NULL);
    _TyErr_Clear(ts);
     ...
     * any trouble. */
    _Ty_AuditHookEntry *e = runtime->audit_hooks.head, *n;
...
add_audit_hook_entry_unlocked(_PyRuntimeState *runtime,
                              _Ty_AuditHookEntry *entry)
{
    if (runtime->audit_hooks.head == NULL) {
        runtime->audit_hooks.head = entry;
    }
    else {
        _Ty_AuditHookEntry *last = runtime->audit_hooks.head;
        while (last->next) {
            last = last->next;
        }
        last->next = entry;
...
int
TySys_AddAuditHook(Ty_AuditHookFunction hook, void *userData)
{
    ...
    if (tstate != NULL) {
        if (_TySys_Audit(tstate, "sys.addaudithook", NULL) < 0) {
            if (_TyErr_ExceptionMatches(tstate, TyExc_RuntimeError)) {
    ...
    _Ty_AuditHookEntry *e = (_Ty_AuditHookEntry*)TyMem_RawMalloc(
            sizeof(_Ty_AuditHookEntry));
    if (!e) {
        if (tstate != NULL) {
    ...
    e->next = NULL;
    e->hookCFunction = (Ty_AuditHookFunction)hook;
    e->userData = userData;
...
/*[clinic end generated code: output=4f9c17aaeb02f44e input=0f3e191217a45e34]*/
{
    ...
    if (TyList_Append(interp->audit_hooks, hook) < 0) {
        return NULL;
    }
...
sys_breakpointhook(TyObject *self, TyObject *const *args, Ty_ssize_t nargs, TyObject *keywords)
{
    ...
    TyObject *modulepath = NULL;

    if (last_dot == NULL) {
        /* The breakpoint is a built-in, e.g. PYTHONBREAKPOINT=int */
        modulepath = TyUnicode_FromString("builtins");
        attrname = envar;
    }
    else if (last_dot != envar) {
        /* Split on the last dot; */
        modulepath = TyUnicode_FromStringAndSize(envar, last_dot - envar);
        attrname = last_dot + 1;
    }
    else {
        goto warn;
    ...
    TyObject *module = TyImport_Import(modulepath);
    Ty_DECREF(modulepath);
    ...
    TyObject *hook = PyObject_GetAttrString(module, attrname);
    Ty_DECREF(module);
  ...
  warn:
    ...
    _TyErr_Clear(tstate);
    int status = TyErr_WarnFormat(
        TyExc_RuntimeWarning, 0,
...
static int
sys_displayhook_unencodable(TyObject *outf, TyObject *o)
{
    TyObject *stdout_encoding = NULL;
    TyObject *encoded, *escaped_str, *repr_str, *buffer, *result;
    const char *stdout_encoding_str;
    int ret;

    stdout_encoding = PyObject_GetAttr(outf, &_Ty_ID(encoding));
    ...
    if (repr_str == NULL)
        goto error;
    encoded = TyUnicode_AsEncodedString(repr_str,
                                        stdout_encoding_str,
                                        "backslashreplace");
    Ty_DECREF(repr_str);
    if (encoded == NULL)
    ...
    if (PyObject_GetOptionalAttr(outf, &_Ty_ID(buffer), &buffer) < 0) {
        Ty_DECREF(encoded);
        goto error;
    }
    if (buffer) {
        result = PyObject_CallMethodOneArg(buffer, &_Ty_ID(write), encoded);
        Ty_DECREF(buffer);
        Ty_DECREF(encoded);
        if (result == NULL)
            goto error;
        ...
        escaped_str = TyUnicode_FromEncodedObject(encoded,
                                                  stdout_encoding_str,
                                                  "strict");
        Ty_DECREF(encoded);
        if (TyFile_WriteObject(escaped_str, outf, Ty_PRINT_RAW) != 0) {
            Ty_DECREF(escaped_str);
            goto error;
        }
        Ty_DECREF(escaped_str);
    }
...
static TyObject *
sys_displayhook(TyObject *module, TyObject *o)
/*[clinic end generated code: output=347477d006df92ed input=08ba730166d7ef72]*/
{
    ...
    if (PyObject_SetAttr(builtins, _Ty_LATIN1_CHR('_'), Ty_None) != 0)
        return NULL;
    outf = _TySys_GetRequiredAttr(&_Ty_ID(stdout));
    if (outf == NULL) {
        return NULL;
    }
    if (outf == Ty_None) {
        _TyErr_SetString(tstate, TyExc_RuntimeError, "lost sys.stdout");
        Ty_DECREF(outf);
        return NULL;
    }
    if (TyFile_WriteObject(o, outf, 0) != 0) {
        if (_TyErr_ExceptionMatches(tstate, TyExc_UnicodeEncodeError)) {
            int err;
    ...
    if (TyFile_WriteObject(_Ty_LATIN1_CHR('\n'), outf, Ty_PRINT_RAW) != 0) {
        Ty_DECREF(outf);
        return NULL;
    }
    Ty_DECREF(outf);
...
{
    _TyErr_StackItem *err_info = _TyErr_GetTopmostException(_TyThreadState_GET());
...
{
    _TyErr_StackItem *err_info = _TyErr_GetTopmostException(_TyThreadState_GET());
    return _TyErr_StackItemToExcInfoTuple(err_info);
}
...
static TyObject *
sys_unraisablehook(TyObject *module, TyObject *unraisable)
/*[clinic end generated code: output=bb92838b32abaa14 input=ec3af148294af8d3]*/
{
    return _TyErr_WriteUnraisableDefaultHook(unraisable);
}
...
/*[clinic end generated code: output=13870986c1ab2ec0 input=b86ca9497baa94f2]*/
{
    ...
    TyErr_SetObject(TyExc_SystemExit, status);
    return NULL;
}
...
/*[clinic end generated code: output=1dc4bdbe9be44aa7 input=8475f8649b8c7d8c]*/
{
    ...
    TyObject *u = TyUnicode_FromWideChar(config->filesystem_encoding, -1);
    if (u == NULL) {
        return NULL;
    }
    _TyUnicode_InternImmortal(interp, &u);
    return u;
}
...
/*[clinic end generated code: output=be680c24f5c9e5d6 input=849483c006924e2f]*/
{
    if (TyUnicode_CheckExact(s)) {
        ...
        TyErr_Format(TyExc_TypeError,
                     "can't intern %.400s", Ty_TYPE(s)->tp_name);
        return NULL;
    }
}
...
{
    return TyUnicode_CHECK_INTERNED(string);
}
                ...
                PyFrameObject *frame, int what, TyObject *arg)
{
    ...
    TyObject *args[3] = {(TyObject *)frame, whatstrings[what], arg};
    TyObject *result = _TyObject_VectorcallTstate(tstate, callback, args, 3, NULL);
...
static int
trace_trampoline(TyObject *self, PyFrameObject *frame,
                 int what, TyObject *arg)
{
    TyObject *callback;
    ...
    if (result == NULL) {
        ...
        Ty_CLEAR(frame->f_trace);
        return -1;
    }

    if (result != Ty_None) {
        Ty_XSETREF(frame->f_trace, result);
    }
    else {
        Ty_DECREF(result);
    }
...
/*[clinic end generated code: output=999d12e9d6ec4678 input=8107feb01c5f1c4e]*/
{
    ...
    if (function == Ty_None) {
        ...
        if (_TyEval_SetTrace(tstate, trace_trampoline, function) < 0) {
            return NULL;
        }
    }
    Py_RETURN_NONE;
...
static TyObject *
sys__settraceallthreads(TyObject *module, TyObject *arg)
/*[clinic end generated code: output=161cca30207bf3ca input=d4bde1f810d73675]*/
{
    TyObject* argument = NULL;
    Ty_tracefunc func = NULL;
...
/*[clinic end generated code: output=1c3503105939db9c input=055d0d7961413a62]*/
{
    ...
    if (function == Ty_None) {
        ...
        if (_TyEval_SetProfile(tstate, profile_trampoline, function) < 0) {
            return NULL;
        }
    }
    Py_RETURN_NONE;
...
static TyObject *
sys__setprofileallthreads(TyObject *module, TyObject *arg)
...
{
    return 1e-6 * _TyEval_GetSwitchInterval();
}
...
{
    return _TyEval_GetCoroutineOriginTrackingDepth();
}
...
static TyObject *
sys_set_asyncgen_hooks(TyObject *self, TyObject *args, TyObject *kw)
{
    static char *keywords[] = {"firstiter", "finalizer", NULL};
    TyObject *firstiter = NULL;
    TyObject *finalizer = NULL;
    ...
    if (finalizer && finalizer != Ty_None) {
        if (!PyCallable_Check(finalizer)) {
            TyErr_Format(TyExc_TypeError,
                         "callable finalizer expected, got %.50s",
                         Ty_TYPE(finalizer)->tp_name);
            return NULL;
        }
    }

    if (firstiter && firstiter != Ty_None) {
        if (!PyCallable_Check(firstiter)) {
            TyErr_Format(TyExc_TypeError,
                         "callable firstiter expected, got %.50s",
                         Ty_TYPE(firstiter)->tp_name);
            return NULL;
        }
    }

    TyObject *cur_finalizer = _TyEval_GetAsyncGenFinalizer();

    if (finalizer && finalizer != Ty_None) {
        if (_TyEval_SetAsyncGenFinalizer(finalizer) < 0) {
            return NULL;
        }
    }
    else if (finalizer == Ty_None && _TyEval_SetAsyncGenFinalizer(NULL) < 0) {
        return NULL;
    }

    if (firstiter && firstiter != Ty_None) {
        if (_TyEval_SetAsyncGenFirstiter(firstiter) < 0) {
            goto error;
        }
    }
    else if (firstiter == Ty_None && _TyEval_SetAsyncGenFirstiter(NULL) < 0) {
        goto error;
    }
...
#define SET_HASH_INFO_ITEM(CALL)                             \
    ...
    SET_HASH_INFO_ITEM(TyLong_FromSsize_t(_PyHASH_MODULUS));
    SET_HASH_INFO_ITEM(TyLong_FromLong(_PyHASH_INF));
    SET_HASH_INFO_ITEM(TyLong_FromLong(0));  // This is no longer used
    SET_HASH_INFO_ITEM(TyLong_FromLong(_PyHASH_IMAG));
    SET_HASH_INFO_ITEM(TyUnicode_FromString(hashfunc->name));
    SET_HASH_INFO_ITEM(TyLong_FromLong(hashfunc->hash_bits));
    SET_HASH_INFO_ITEM(TyLong_FromLong(hashfunc->seed_bits));
    SET_HASH_INFO_ITEM(TyLong_FromLong(Ty_HASH_CUTOFF));
...
{
    return TyLong_FromLong(Ty_GetRecursionLimit());
}
...
_sys_getwindowsversion_from_kernel32(void)
{
    ...
    TyMem_RawFree(verblock);
    return Ty_BuildValue("(kkk)", realMajor, realMinor, realBuild);
...
#define SET_VERSION_INFO(CALL)                               \
    ...
    SET_VERSION_INFO(TyLong_FromLong(ver.dwMinorVersion));
    SET_VERSION_INFO(TyLong_FromLong(ver.dwBuildNumber));
    SET_VERSION_INFO(TyLong_FromLong(ver.dwPlatformId));
    SET_VERSION_INFO(TyUnicode_FromWideChar(ver.szCSDVersion, -1));
    SET_VERSION_INFO(TyLong_FromLong(ver.wServicePackMajor));
    SET_VERSION_INFO(TyLong_FromLong(ver.wServicePackMinor));
    SET_VERSION_INFO(TyLong_FromLong(ver.wSuiteMask));
    SET_VERSION_INFO(TyLong_FromLong(ver.wProductType));

    // GetVersion will lie if we are running in a compatibility mode.
    // We need to read the version info from a system file resource
    // to accurately identify the OS version. If we fail for any reason,
    // just return whatever GetVersion said.
    TyObject *realVersion = _sys_getwindowsversion_from_kernel32();
    if (!realVersion) {
        if (!TyErr_ExceptionMatches(TyExc_WindowsError)) {
            return NULL;
        }

        TyErr_Clear();
        realVersion = Ty_BuildValue("(kkk)",
            ver.dwMajorVersion,
            ver.dwMinorVersion,
            ver.dwBuildNumber
        );
...
_TySys_GetSizeOf(TyObject *o)
{
    ...
    method = _TyObject_LookupSpecial(o, &_Ty_ID(__sizeof__));
    if (method == NULL) {
        if (!_TyErr_Occurred(tstate)) {
        ...
        res = _TyObject_CallNoArgs(method);
        Ty_DECREF(method);
    }
    ...
    size = TyLong_AsSsize_t(res);
    Ty_DECREF(res);
    if (size == -1 && _TyErr_Occurred(tstate))
        return (size_t)-1;
    ...
    size_t presize = 0;
    if (!Ty_IS_TYPE(o, &TyType_Type) ||
         TyType_HasFeature((TyTypeObject *)o, Ty_TPFLAGS_HEAPTYPE))
    {
        /* Add the size of the pre-header if "o" is not a static type */
        presize = _TyType_PreHeaderSize(Ty_TYPE(o));
    }
...
static TyObject *
sys_getsizeof(TyObject *self, TyObject *args, TyObject *kwds)
{
    ...
    size = _TySys_GetSizeOf(o);

    if (size == (size_t)-1 && _TyErr_Occurred(tstate)) {
        /* Has a default value been given */
        if (dflt != NULL && _TyErr_ExceptionMatches(tstate, TyExc_TypeError)) {
            _TyErr_Clear(tstate);
...
{
    return Ty_REFCNT(object);
}

#ifdef Ty_REF_DEBUG
/*[clinic input]
sys.gettotalrefcount -> Ty_ssize_t
[clinic start generated code]*/
...
/*[clinic end generated code: output=4103886cf17c25bc input=53b744faa5d2e4f6]*/
{
    ...
    return _Ty_GetGlobalRefTotal();
}

#endif /* Ty_REF_DEBUG */

/*[clinic input]
sys.getallocatedblocks -> Ty_ssize_t
...
/*[clinic end generated code: output=f0c4e873f0b6dcf7 input=dab13ee346a0673e]*/
{
    ...
    return _Ty_GetGlobalAllocatedBlocks();
}

/*[clinic input]
sys.getunicodeinternedsize -> Ty_ssize_t
...
{
    if (_only_immortal) {
        return _TyUnicode_InternedSize_Immortal();
    }
    else {
        return _TyUnicode_InternedSize();
    }
}
...
/*[clinic end generated code: output=d438776c04d59804 input=c1be8a6464b11ee5]*/
{
    ...
    TyObject *pyFrame = Ty_XNewRef((TyObject *)_TyFrame_GetFrameObject(frame));
    if (pyFrame && _TySys_Audit(tstate, "sys._getframe", "(O)", pyFrame) < 0) {
        Ty_DECREF(pyFrame);
        return NULL;
    }
    return pyFrame;
...
sys.call_tracing
    ...
    func: object
    args as funcargs: object(subclass_of='&TyTuple_Type')
    /
...
{
    return _TyEval_CallTracing(func, funcargs);
}
...
{
#ifdef WITH_PYMALLOC
    if (_TyObject_DebugMallocStats(stderr)) {
        fputc('\n', stderr);
    }
#endif
...
extern TyObject *_Ty_GetObjects(TyObject *, TyObject *);
#endif
...
/*[clinic end generated code: output=0ee128670a4966d6 input=253e741ca744f6e8]*/
{
    ...
    if (_Ty_ClearUnusedTLBC(_TyInterpreterState_GET()) < 0) {
        return NULL;
    }
#endif
    TyType_ClearCache();
...
{
    return TyBool_FromLong(Ty_IsFinalizing());
}
...
{
    int res = _Ty_PrintSpecializationStats(1);
    _Ty_StatsClear();
    return res;
}
#endif   // Ty_STATS
...
{
    return TyLong_FromLong(ANDROID_API_LEVEL);
}
...
/*[clinic end generated code: output=5783cdeb51874b43 input=a12df928758a82b4]*/
{
...
#ifdef _Ty_JIT
    if (_TyInterpreterState_GET()->jit) {
        TyErr_SetString(TyExc_ValueError, "Cannot activate the perf trampoline if the JIT is active");
        return NULL;
    }
#endif
    ...
    if (strcmp(backend, "perf") == 0) {
        ...
        if (cur_cb.write_state != _Ty_perfmap_callbacks.write_state) {
            if (_PyPerfTrampoline_SetCallbacks(&_Ty_perfmap_callbacks) < 0 ) {
                TyErr_SetString(TyExc_ValueError, "can't activate perf trampoline");
                return NULL;
            }
        }
            ...
            _PyPerf_Callbacks cur_cb;
            _PyPerfTrampoline_GetCallbacks(&cur_cb);
            if (cur_cb.write_state != _Ty_perfmap_jit_callbacks.write_state) {
                if (_PyPerfTrampoline_SetCallbacks(&_Ty_perfmap_jit_callbacks) < 0 ) {
                    TyErr_SetString(TyExc_ValueError, "can't activate perf jit trampoline");
                    return NULL;
                }
            }
        }
        ...
        TyErr_Format(TyExc_ValueError, "invalid backend: %s", backend);
        return NULL;
    }
...
/*[clinic end generated code: output=7d94c56afe4a52c0 input=39908ca2c5fe1eb0]*/
{
    ...
    if (TyUnicode_FSConverter(script, &path) == 0) {
        return NULL;
    }

    if (TySys_Audit("sys.remote_exec", "iO", pid, script) < 0) {
        return NULL;
    }

    debugger_script_path = TyBytes_AS_STRING(path);
#ifdef MS_WINDOWS
    TyObject *unicode_path;
    if (TyUnicode_FSDecoder(path, &unicode_path) < 0) {
        goto error;
    }
    // Use UTF-16 (wide char) version of the path for permission checks
    wchar_t *debugger_script_path_w = TyUnicode_AsWideCharString(unicode_path, NULL);
    Ty_DECREF(unicode_path);
    ...
    if (attr == INVALID_FILE_ATTRIBUTES) {
        ...
        TyMem_Free(debugger_script_path_w);
        if (err == ERROR_FILE_NOT_FOUND || err == ERROR_PATH_NOT_FOUND) {
            TyErr_SetString(TyExc_FileNotFoundError, "Script file does not exist");
        }
        else if (err == ERROR_ACCESS_DENIED) {
            TyErr_SetString(TyExc_PermissionError, "Script file cannot be read");
        }
        else {
...
{
    if (TySys_Audit("sys._getframemodulename", "i", depth) < 0) {
        return NULL;
    }
    _PyInterpreterFrame *f = _TyThreadState_GET()->current_frame;
    while (f && (_TyFrame_IsIncomplete(f) || depth-- > 0)) {
        f = f->previous;
    }
...
{
#ifdef Ty_GIL_DISABLED
    return _TyEval_IsGILEnabled(_TyThreadState_GET());
#else
    return 1;
#endif
}
...
static TyMethodDef sys_methods[] = {
...
#ifdef Ty_TRACE_REFS
    {"getobjects", _Ty_GetObjects, METH_VARARGS},
#endif
...
{
    TyObject *list = _TyImport_GetBuiltinModuleNames();
    if (list == NULL) {
        return NULL;
    }
    if (TyList_Sort(list) != 0) {
        goto error;
    }
    TyObject *tuple = TyList_AsTuple(list);
    Ty_DECREF(list);
    return tuple;
...
{
    Ty_ssize_t len = Ty_ARRAY_LENGTH(_Ty_stdlib_module_names);
    TyObject *names = TyTuple_New(len);
    if (names == NULL) {
        return NULL;
    }

    for (Ty_ssize_t i = 0; i < len; i++) {
        TyObject *name = TyUnicode_FromString(_Ty_stdlib_module_names[i]);
        if (name == NULL) {
            Ty_DECREF(names);
            return NULL;
        }
        TyTuple_SET_ITEM(names, i, name);
    }

    TyObject *set = PyObject_CallFunction((TyObject *)&TyFrozenSet_Type,
                                          "(O)", names);
    Ty_DECREF(names);
    return set;
}
 ...
 *   These APIs get called after _Ty_InitializeCore and get to use the
 ...
 *   before they call Ty_Initialize. To support this, we stash copies of
...
typedef struct _preinit_entry *_Ty_PreInitEntry;

static _Ty_PreInitEntry _preinit_warnoptions = NULL;
static _Ty_PreInitEntry _preinit_xoptions = NULL;

static _Ty_PreInitEntry
_alloc_preinit_entry(const wchar_t *value)
{
     ...
     */
    _Ty_PreInitEntry node = _TyMem_DefaultRawCalloc(1, sizeof(*node));
    if (node != NULL) {
        node->value = _TyMem_DefaultRawWcsdup(value);
        if (node->value == NULL) {
            _TyMem_DefaultRawFree(node);
            node = NULL;
        };
    };
    return node;
...
static int
_append_preinit_entry(_Ty_PreInitEntry *optionlist, const wchar_t *value)
{
    _Ty_PreInitEntry new_entry = _alloc_preinit_entry(value);
    if (new_entry == NULL) {
        return -1;
    }
    /* We maintain the linked list in this order so it's easy to play back
     * the add commands in the same order later on in _Ty_InitializeCore
     */
    _Ty_PreInitEntry last_entry = *optionlist;
    if (last_entry == NULL) {
...
static void
_clear_preinit_entries(_Ty_PreInitEntry *optionlist)
{
    _Ty_PreInitEntry current = *optionlist;
    *optionlist = NULL;
    /* Deallocate the nodes and their contents using the default allocator */
    while (current != NULL) {
        _Ty_PreInitEntry next = current->next;
        _TyMem_DefaultRawFree(current->value);
        _TyMem_DefaultRawFree(current);
        current = next;
    }
}
...
_TySys_ReadPreinitWarnOptions(TyWideStringList *options)
{
    ...
    for (entry = _preinit_warnoptions; entry != NULL; entry = entry->next) {
        status = TyWideStringList_Append(options, entry->value);
...
_TySys_ReadPreinitXOptions(TyConfig *config)
{
    ...
    for (entry = _preinit_xoptions; entry != NULL; entry = entry->next) {
        status = TyWideStringList_Append(&config->xoptions, entry->value);
        if (_TyStatus_EXCEPTION(status)) {
            return status;
        }
    }
...
get_warnoptions(TyThreadState *tstate)
{
    ...
    if (warnoptions == NULL || !TyList_Check(warnoptions)) {
        ...
        *  interpreter config. When that happens, we need to properly set
         ...
         * combination of how _PyMainInterpreter_ReadConfig and _TySys_EndInit
         * work, but we expect 3.8+ to make the _PyMainInterpreter_ReadConfig
         * call optional for embedding applications, thus making this
         * reachable again.
         */
        Ty_XDECREF(warnoptions);
        warnoptions = TyList_New(0);
        if (warnoptions == NULL) {
            return NULL;
        }
        if (sys_set_object(tstate->interp, &_Ty_ID(warnoptions), warnoptions)) {
            Ty_DECREF(warnoptions);
            return NULL;
        }
    }
    return warnoptions;
...
TySys_ResetWarnOptions(void)
{
    ...
    if (warnoptions != NULL && TyList_Check(warnoptions)) {
        TyList_SetSlice(warnoptions, 0, TyList_GET_SIZE(warnoptions), NULL);
    }
    Ty_XDECREF(warnoptions);
}
...
_TySys_AddWarnOptionWithError(TyThreadState *tstate, TyObject *option)
{
    ...
    if (TyList_Append(warnoptions, option)) {
        Ty_DECREF(warnoptions);
        return -1;
    }
    Ty_DECREF(warnoptions);
...
TySys_AddWarnOption(const wchar_t *s)
{
    ...
    TyObject *unicode;
    unicode = TyUnicode_FromWideChar(s, -1);
    if (unicode == NULL)
        return;
_Ty_COMP_DIAG_PUSH
_Ty_COMP_DIAG_IGNORE_DEPR_DECLS
    TySys_AddWarnOptionUnicode(unicode);
_Ty_COMP_DIAG_POP
    Ty_DECREF(unicode);
}
...
TySys_HasWarnOptions(void)
{
    ...
    int r = (warnoptions != NULL && TyList_Check(warnoptions) &&
             TyList_GET_SIZE(warnoptions) > 0);
    Ty_XDECREF(warnoptions);
    return r;
}
...
get_xoptions(TyThreadState *tstate)
{
    ...
    if (xoptions == NULL || !TyDict_Check(xoptions)) {
        ...
        *  interpreter config. When that happens, we need to properly set
        ...
        if (sys_set_object(tstate->interp, &_Ty_ID(_xoptions), xoptions)) {
            Ty_DECREF(xoptions);
            return NULL;
        }
    }
...
static int
_TySys_AddXOptionWithError(const wchar_t *s)
{
    ...
    if (!name_end) {
        name = TyUnicode_FromWideChar(s, -1);
        if (name == NULL) {
            goto error;
        }
        ...
        name = TyUnicode_FromWideChar(s, name_end - s);
        if (name == NULL) {
            goto error;
        }
    ...
    if (TyDict_SetItem(opts, name, value) < 0) {
        goto error;
    }
    Ty_DECREF(name);
    Ty_DECREF(value);
...
TySys_AddXOption(const wchar_t *s)
{
    ...
    if (_TySys_AddXOptionWithError(s) < 0) {
        /* No return value, therefore clear error state if possible */
        _TyErr_Clear(tstate);
    }
}
...
static void
sys_set_flag(TyObject *flags, Ty_ssize_t pos, TyObject *value)
{
...
int
_TySys_SetFlagObj(Ty_ssize_t pos, TyObject *value)
{
    TyObject *flags = _TySys_GetRequiredAttrString("flags");
    if (flags == NULL) {
        return -1;
    }
...
static int
_TySys_SetFlagInt(Ty_ssize_t pos, int value)
{
    TyObject *obj = TyLong_FromLong(value);
    if (obj == NULL) {
        return -1;
    }

    int res = _TySys_SetFlagObj(pos, obj);
    Ty_DECREF(obj);
    return res;
}
...
#define SetFlag(expr) SetFlagObj(TyLong_FromLong(expr))
    ...
    SetFlagObj(TyBool_FromLong(config->dev_mode));
    SetFlag(preconfig->utf8_mode);
    SetFlag(config->warn_default_encoding);
    SetFlagObj(TyBool_FromLong(config->safe_path));
    SetFlag(config->int_max_str_digits);
#ifdef Ty_GIL_DISABLED
    if (config->enable_gil == _TyConfig_GIL_DEFAULT) {
        SetFlagObj(Ty_NewRef(Ty_None));
    }
    else {
        SetFlag(config->enable_gil);
    }
#else
    SetFlagObj(TyLong_FromLong(1));
#endif
...
#define NAME "cpython"
const char *_TySys_ImplName = NAME;
#define MAJOR Ty_STRINGIFY(PY_MAJOR_VERSION)
#define MINOR Ty_STRINGIFY(PY_MINOR_VERSION)
#define TAG NAME "-" MAJOR MINOR
const char *_TySys_ImplCacheTag = TAG;
#undef NAME
#undef MAJOR
#undef MINOR
#undef TAG
...
static TyObject *
make_impl_info(TyObject *version_info)
{
    int res;
    TyObject *impl_info, *value, *ns;

    impl_info = TyDict_New();
    if (impl_info == NULL)
        return NULL;

    /* populate the dict */

    value = TyUnicode_FromString(_TySys_ImplName);
    if (value == NULL)
        goto error;
    res = TyDict_SetItemString(impl_info, "name", value);
    Ty_DECREF(value);
    if (res < 0)
        goto error;

    value = TyUnicode_FromString(_TySys_ImplCacheTag);
    if (value == NULL)
        goto error;
    res = TyDict_SetItemString(impl_info, "cache_tag", value);
    Ty_DECREF(value);
    if (res < 0)
        goto error;

    res = TyDict_SetItemString(impl_info, "version", version_info);
    if (res < 0)
        goto error;

    value = TyLong_FromLong(PY_VERSION_HEX);
    if (value == NULL)
        goto error;
    res = TyDict_SetItemString(impl_info, "hexversion", value);
    Ty_DECREF(value);
    if (res < 0)
        goto error;

#ifdef MULTIARCH
    value = TyUnicode_FromString(MULTIARCH);
    if (value == NULL)
        goto error;
    res = TyDict_SetItemString(impl_info, "_multiarch", value);
    Ty_DECREF(value);
    if (res < 0)
        goto error;
#endif
    ...
    value = Ty_True;
#endif
    res = TyDict_SetItemString(impl_info, "supports_isolated_interpreters", value);
    if (res < 0) {
        goto error;
    }
...
EM_JS(char *, _Ty_emscripten_runtime, (void), {
    var info;
...
{
    TyObject *emscripten_info = NULL;
    TyObject *version = NULL;
    char *ua;
    int pos = 0;
    ...
    if (ua != NULL) {
        TyObject *oua = TyUnicode_DecodeUTF8(ua, strlen(ua), "strict");
        free(ua);
        if (oua == NULL) {
            goto error;
        }
...
#undef SetBoolItem
  ...
  error:
    Ty_CLEAR(emscripten_info);
    return NULL;
}

#endif // __EMSCRIPTEN__

static struct TyModuleDef sysmodule = {
...
#define SET_SYS(key, value)                                \
    do {                                                   \
        ...
        res = TyDict_SetItemString(sysdict, key, v);       \
        Ty_DECREF(v);                                      \
        if (res < 0) {                                     \
            goto err_occurred;                             \
        }                                                  \
...
#undef COPY_SYS_ATTR
    ...
    SET_SYS_FROM_STRING("version", Ty_GetVersion());
    SET_SYS("hexversion", TyLong_FromLong(PY_VERSION_HEX));
    SET_SYS("_git", Ty_BuildValue("(szz)", "CPython", _Ty_gitidentifier(),
                                  _Ty_gitversion()));
    SET_SYS_FROM_STRING("_framework", _PYTHONFRAMEWORK);
    SET_SYS("api_version", TyLong_FromLong(PYTHON_API_VERSION));
    SET_SYS_FROM_STRING("copyright", Ty_GetCopyright());
    SET_SYS_FROM_STRING("platform", Ty_GetPlatform());
    SET_SYS("maxsize", TyLong_FromSsize_t(PY_SSIZE_T_MAX));
    SET_SYS("float_info", TyFloat_GetInfo());
    SET_SYS("int_info", TyLong_GetInfo());
    ...
    SET_SYS("maxunicode", TyLong_FromLong(0x10FFFF));
...
#ifdef MS_COREDLL
    SET_SYS("dllhandle", TyLong_FromVoidPtr(PyWin_DLLhModule));
...
#define ENSURE_INFO_TYPE(TYPE, DESC) \
    do { \
        if (_PyStructSequence_InitBuiltinWithFlags( \
                interp, &TYPE, &DESC, Ty_TPFLAGS_DISALLOW_INSTANTIATION) < 0) { \
            goto type_init_failed; \
        } \
    } while (0)
    ...
    // sys.flags: updated in-place later by _TySys_UpdateConfig()
...
#undef ENSURE_INFO_TYPE
    ...
    SET_SYS("meta_path", TyList_New(0));
    SET_SYS("path_importer_cache", TyDict_New());
    SET_SYS("path_hooks", TyList_New(0));
...
type_init_failed:
    return _TyStatus_ERR("failed to initialize a type");

err_occurred:
    return _TyStatus_ERR("can't initialize sys module");
}
...
#define COPY_LIST(KEY, VALUE) \
        SET_SYS(KEY, _TyWideStringList_AsList(&(VALUE)));

#define SET_SYS_FROM_WSTR(KEY, VALUE) \
        SET_SYS(KEY, TyUnicode_FromWideChar(VALUE, -1));
...
#define COPY_WSTR(SYS_ATTR, WSTR) \
    ...
    if (config->pycache_prefix != NULL) {
        ...
        if (TyDict_SetItemString(sysdict, "pycache_prefix", Ty_None) < 0) {
            return -1;
        }
    }
    ...
    SET_SYS("_xoptions", _TyConfig_CreateXOptionsDict(config));

    const wchar_t *stdlibdir = _Ty_GetStdlibDir();
    if (stdlibdir != NULL) {
        SET_SYS_FROM_WSTR("_stdlib_dir", stdlibdir);
    }
    else {
        if (TyDict_SetItemString(sysdict, "_stdlib_dir", Ty_None) < 0) {
            return -1;
        }
    }
...
#undef COPY_WSTR
    ...
    SET_SYS("dont_write_bytecode", TyBool_FromLong(!config->write_bytecode));
...
_TySys_SetPreliminaryStderr(TyObject *sysdict)
{
    ...
    if (TyDict_SetItem(sysdict, &_Ty_ID(stderr), pstderr) < 0) {
        goto error;
    }
    if (TyDict_SetItemString(sysdict, "__stderr__", pstderr) < 0) {
        goto error;
    }
    Ty_DECREF(pstderr);
    return _TyStatus_OK();
...
error:
    Ty_XDECREF(pstderr);
    return _TyStatus_ERR("can't set preliminary stderr");
}
...
{
    (void)module;
    return _TyInterpreterState_GET()->jit;
}
...
{
    (void)module;
    return _TyThreadState_GET()->current_executor != NULL;
}

static TyMethodDef _jit_methods[] = {
...
static struct TyModuleDef _jit_module = {
    PyModuleDef_HEAD_INIT,
    .m_name = "sys._jit",
    .m_doc = _jit_doc,
    .m_size = -1,
...
_TySys_Create(TyThreadState *tstate, TyObject **sysmod_p)
{
    ...
    TyObject *sysmod = _TyModule_CreateInitialized(&sysmodule, PYTHON_API_VERSION);
    if (sysmod == NULL) {
        return _TyStatus_ERR("failed to create a module object");
    }
#ifdef Ty_GIL_DISABLED
    PyUnstable_Module_SetGIL(sysmod, Ty_MOD_GIL_NOT_USED);
#endif

    TyObject *sysdict = TyModule_GetDict(sysmod);
    if (sysdict == NULL) {
        goto error;
    }
    interp->sysdict = Ty_NewRef(sysdict);

    interp->sysdict_copy = TyDict_Copy(sysdict);
    if (interp->sysdict_copy == NULL) {
        goto error;
    }

    if (TyDict_SetItemString(sysdict, "modules", modules) < 0) {
        goto error;
    }
    ...
    status = _TySys_InitCore(tstate, sysdict);
    if (_TyStatus_EXCEPTION(status)) {
        return status;
    }

    if (_TyImport_FixupBuiltin(tstate, sysmod, "sys", modules) < 0) {
        goto error;
    }

    TyObject *monitoring = _Ty_CreateMonitoringObject();
    if (monitoring == NULL) {
        goto error;
    }
    int err = TyDict_SetItemString(sysdict, "monitoring", monitoring);
    Ty_DECREF(monitoring);
    if (err < 0) {
        goto error;
    }

    TyObject *_jit = _TyModule_CreateInitialized(&_jit_module, PYTHON_API_VERSION);
    if (_jit == NULL) {
        goto error;
    }
    err = TyDict_SetItemString(sysdict, "_jit", _jit);
    Ty_DECREF(_jit);
    if (err) {
        goto error;
    }
...
makepathobject(const wchar_t *path, wchar_t delim)
{
    ...
    for (i = 0; ; i++) {
        ...
        w = TyUnicode_FromWideChar(path, (Ty_ssize_t)(p - path));
        if (w == NULL) {
            Ty_DECREF(v);
            return NULL;
        }
...
// Removed in Python 3.13 API, but kept for the stable ABI
PyAPI_FUNC(void)
TySys_SetPath(const wchar_t *path)
{
    TyObject *v;
...
{
    TyObject *list = TyList_New(argc);
    if (list == NULL) {
        return NULL;
    }

    for (Ty_ssize_t i = 0; i < argc; i++) {
        TyObject *v = TyUnicode_FromWideChar(argv[i], -1);
        if (v == NULL) {
            Ty_DECREF(list);
            return NULL;
        }
...
TySys_SetArgvEx(int argc, wchar_t **argv, int updatepath)
{
    ...
    TyObject *av = make_sys_argv(argc, argv);
    if (av == NULL) {
        Ty_FatalError("no mem for sys.argv");
    }
    if (sys_set_object_str(tstate->interp, "argv", av) != 0) {
        Ty_DECREF(av);
        Ty_FatalError("can't assign sys.argv");
    }
    Ty_DECREF(av);
    ...
    if (updatepath) {
        ...
        if (_TyPathConfig_ComputeSysPath0(&argv_list, &path0)) {
            if (path0 == NULL) {
                Ty_FatalError("can't compute path0 from argv");
            }

            TyObject *sys_path;
            if (_TySys_GetOptionalAttr(&_Ty_ID(path), &sys_path) < 0) {
                Ty_FatalError("can't get sys.path");
            }
            else if (sys_path != NULL) {
                if (TyList_Insert(sys_path, 0, path0) < 0) {
                    Ty_FatalError("can't prepend path0 to sys.path");
                }
                Ty_DECREF(sys_path);
            }
            Ty_DECREF(path0);
...
void
TySys_SetArgv(int argc, wchar_t **argv)
{
_Ty_COMP_DIAG_PUSH
_Ty_COMP_DIAG_IGNORE_DEPR_DECLS
    TySys_SetArgvEx(argc, argv, Ty_IsolatedFlag == 0);
_Ty_COMP_DIAG_POP
}

/* Reimplementation of TyFile_WriteString() no calling indirectly
   TyErr_CheckSignals(): avoid the call to PyObject_Str(). */

static int
sys_pyfile_write_unicode(TyObject *unicode, TyObject *file)
{
    if (file == NULL)
        return -1;
    assert(unicode != NULL);
    TyObject *result = PyObject_CallMethodOneArg(file, &_Ty_ID(write), unicode);
    if (result == NULL) {
        return -1;
    }
    Ty_DECREF(result);
...
static int
sys_pyfile_write(const char *text, TyObject *file)
{
    TyObject *unicode = NULL;
    int err;

    if (file == NULL)
        return -1;

    unicode = TyUnicode_FromString(text);
    if (unicode == NULL)
        return -1;
...
/* APIs to write to sys.stdout or sys.stderr using a printf-like interface.
   ...
   TySys_WriteStdout(format, ...)
   TySys_WriteStderr(format, ...)

      The first function writes to sys.stdout; the second to sys.stderr.  When
      there is a problem, they write to the real (C level) stdout or stderr;
      no exceptions are raised.

      TyErr_CheckSignals() is not called to avoid the execution of the Python
...
static void
sys_write(TyObject *key, FILE *fp, const char *format, va_list va)
{
    TyObject *file;
    char buffer[1001];
    int written;
    ...
    file = _TySys_GetRequiredAttr(key);
...
void
TySys_WriteStdout(const char *format, ...)
{
    va_list va;

    va_start(va, format);
    sys_write(&_Ty_ID(stdout), stdout, format, va);
    va_end(va);
}

void
TySys_WriteStderr(const char *format, ...)
{
    va_list va;

    va_start(va, format);
    sys_write(&_Ty_ID(stderr), stderr, format, va);
    va_end(va);
}

static void
sys_format(TyObject *key, FILE *fp, const char *format, va_list va)
{
...
void
TySys_FormatStdout(const char *format, ...)
{
    va_list va;

    va_start(va, format);
    sys_format(&_Ty_ID(stdout), stdout, format, va);
    va_end(va);
}

void
TySys_FormatStderr(const char *format, ...)
{
    va_list va;

    va_start(va, format);
    sys_format(&_Ty_ID(stderr), stderr, format, va);
    va_end(va);
}


int
_TySys_SetIntMaxStrDigits(int maxdigits)
{
    if (maxdigits != 0 && maxdigits < _PY_LONG_MAX_STR_DIGITS_THRESHOLD) {
        TyErr_Format(
            TyExc_ValueError, "maxdigits must be >= %d or 0 for unlimited",
            _PY_LONG_MAX_STR_DIGITS_THRESHOLD);
        return -1;
    }

    // Set sys.flags.int_max_str_digits
    const Ty_ssize_t pos = SYS_FLAGS_INT_MAX_STR_DIGITS;
    if (_TySys_SetFlagInt(pos, maxdigits) < 0) {
        return -1;
    }
