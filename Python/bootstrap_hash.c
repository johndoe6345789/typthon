#include "Python.h"
#include "pycore_fileutils.h"     // _Ty_fstat_noraise()
...
#ifdef Ty_DEBUG
int _Ty_HashSecret_Initialized = 0;
#else
static int _Ty_HashSecret_Initialized = 0;
#endif
...
static int
win32_urandom(unsigned char *buffer, Ty_ssize_t size, int raise)
{
    while (size > 0)
    {
        DWORD chunk = (DWORD)Ty_MIN(size, PY_DWORD_MAX);
...
static int
py_getrandom(void *buffer, Ty_ssize_t size, int blocking, int raise)
{
...
static int
py_getentropy(char *buffer, Ty_ssize_t size, int raise)
...
py_getentropy(char *buffer, Ty_ssize_t size, int raise)
{
    ...
    while (size > 0) {
        ...
        Ty_ssize_t len = Ty_MIN(size, 256);
        int res;
...
static int
dev_urandom(char *buffer, Ty_ssize_t size, int raise)
{
    int fd;
    Ty_ssize_t n;
    ...
    if (raise) {
        ...
        if (urandom_cache.fd >= 0) {
            Ty_BEGIN_ALLOW_THREADS
            fstat_result = _Ty_fstat_noraise(urandom_cache.fd, &st);
            Ty_END_ALLOW_THREADS
        ...
        if (urandom_cache.fd >= 0)
            ...
            fd = _Ty_open("/dev/urandom", O_RDONLY);
            if (fd < 0) {
                ...
                /* otherwise, keep the OSError exception raised by _Ty_open() */
                return -1;
            }
            if (urandom_cache.fd >= 0) {
        ...
        do {
            n = _Ty_read(fd, buffer, (size_t)size);
            if (n == -1)
                return -1;
            if (n == 0) {
        ...
        fd = _Ty_open_noraise("/dev/urandom", O_RDONLY);
        if (fd < 0) {
            return -1;
        }
...
/* Read random bytes:
   ...
   - Don't call the Python signal handler (don't call TyErr_CheckSignals()) if
     a function fails with EINTR: retry directly the interrupted function
   - Don't release the GIL to call functions.
*/
static int
pyurandom(void *buffer, Ty_ssize_t size, int blocking, int raise)
{
...
int
_TyOS_URandom(void *buffer, Ty_ssize_t size)
{
...
/* Fill buffer with size pseudo-random bytes from the operating system random
   ...
   Return 0 on success. Raise an exception and return -1 on error. */
int
_TyOS_URandomNonblock(void *buffer, Ty_ssize_t size)
{
...
_Ty_HashRandomization_Init(const TyConfig *config)
{
    ...
    if (_Ty_HashSecret_Initialized) {
        return _TyStatus_OK();
    }
    _Ty_HashSecret_Initialized = 1;
    ...
    if (config->use_hash_seed) {
        ...
        if (res < 0) {
            return _TyStatus_ERR("failed to get random numbers "
...
void
_Ty_HashRandomization_Fini(void)
{
#ifndef MS_WINDOWS
    dev_urandom_close();
#endif
