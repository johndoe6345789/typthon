...
function that runs the evaluation loop is the same (_TyEval_EvalFrameDefault)
...
calls the original Python evaluation function (_TyEval_EvalFrameDefault) and
...
extern void *_Ty_trampoline_func_start;  // Start of the template of the
                                         // assembly trampoline
extern void *
    _Ty_trampoline_func_end;  // End of the template of the assembly trampoline
...
_PyPerf_Callbacks _Ty_perfmap_callbacks = {
    &perf_map_init_state,
...
new_code_arena(void)
{
    ...
    if (memory == MAP_FAILED) {
        ...
        TyErr_FormatUnraisable("Failed to create new mmap for perf trampoline");
        perf_status = PERF_STATUS_FAILED;
        return -1;
    }
    void *start = &_Ty_trampoline_func_start;
    void *end = &_Ty_trampoline_func_end;
    ...
    if (res == -1) {
        ...
        TyErr_FormatUnraisable("Failed to set mmap for perf trampoline to "
                               "PROT_READ | PROT_EXEC");
        return -1;
    }
    ...
    if (new_arena == NULL) {
        TyErr_NoMemory();
        munmap(memory, mem_size);
        TyErr_FormatUnraisable("Failed to allocate new code arena struct for perf trampoline");
        return -1;
    }
                        ...
                        int throw)
{
    ...
    int ret = _TyCode_GetExtra((TyObject *)co, extra_code_index, (void **)&f);
    if (ret != 0 || f == NULL) {
        ...
        _TyCode_SetExtra((TyObject *)co, extra_code_index,
                         (void *)new_trampoline);
        f = new_trampoline;
    }
    assert(f != NULL);
    return f(ts, frame, throw, _TyEval_EvalFrameDefault);
default_eval:
    // Something failed, fall back to the default evaluator.
    return _TyEval_EvalFrameDefault(ts, frame, throw);
}
...
int PyUnstable_PerfTrampoline_CompileCode(PyCodeObject *co)
{
    ...
    if (ret != 0 || f == NULL) {
        ...
        trampoline_api.write_state(trampoline_api.state, new_trampoline,
                                   perf_code_arena->code_size, co);
        return _TyCode_SetExtra((TyObject *)co, extra_code_index,
                         (void *)new_trampoline);
    }
#endif // PY_HAVE_PERF_TRAMPOLINE
    return 0;
...
_PyPerfTrampoline_Init(int activate)
{
    ...
    if (!activate) {
        ...
        _TyInterpreterState_SetEvalFrameFunc(tstate->interp, py_trampoline_evaluator);
        extra_code_index = _TyEval_RequestCodeExtraIndex(NULL);
...
_PyPerfTrampoline_AfterFork_Child(void)
{
    ...
    if (persist_after_fork) {
        if (perf_trampoline_type != PERF_TRAMPOLINE_TYPE_MAP) {
            return TyStatus_Error("Failed to copy perf map file as perf trampoline type is not type map.");
        }
        _PyPerfTrampoline_Fini();
        char filename[256];
        ...
        if (PyUnstable_CopyPerfMapFile(filename) != 0) {
            return TyStatus_Error("Failed to copy perf map file.");
        }
    } else {
